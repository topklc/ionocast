...
export function fetchSwpc3day() {
  return tproxy("/api/swpc-3day").then(function(txt) {
    ...
    // The SWPC table is slot-major (rows = 3-h periods, columns = days).
    // Accumulate per-day and concatenate after the parse so `forecast`
    // comes out chronological; pushing straight from the table order
    // interleaved the three days and scrambled the outlook-kp timeline.
    var kpByDay = [[], [], []];
    var inKp = false;
    for (var i = 0; i < lines.length; i++) {
      ...
      if (inKp && dayLabels.length) {
        // Cells may carry a G-scale annotation after the value, e.g.
        // "4.67 (G1)". Without the optional (G#) groups those rows -
        // exactly the storm periods - failed the match and were
        // silently dropped from the forecast and the G1+ prob rows.
        var m = ln.match(/^\s*(\d{2})-(\d{2})UT\s+([\d.]+)(?:\s*\(G\d\))?\s+([\d.]+)(?:\s*\(G\d\))?\s+([\d.]+)/);
        if (m) {
          var slot = m[1];
          ...
          for (var j = 0; j < 3; j++) {
            ...
            kpByDay[j].push({
              utc: day + "/" + slot, day: day, hour: slot,
              endHour: parseInt(m[2], 10), dayLabel: dayLabels[j], kp: kps[j]
            });
          }
        }
      }
    ...
    kpRows = kpByDay[0].concat(kpByDay[1], kpByDay[2]);
    if (kpRows.length) {
...
// Fetch hourly Kyoto Dst quicklook from the SWPC mirror.
// Returns { dst, symH, ae, when, history } where:
//   - dst:     most recent hourly value (nT, negative = storm), or null
//   - symH/ae: reserved (null); drivers.js falls back to showing dst as
//              a Sym-H proxy while symH is null
//   - history: trailing 48 h as [{ t: ms_since_epoch, v: nT }, ...]
//              oldest-first, for the Sym-H sparkline (reads only .v)
export function fetchDst() {
  return jproxy("/api/swpc-dst").then(function(data) {
    var series = [];
    for (var i = 0; i < (data || []).length; i++) {
      var row = data[i]; if (!row) continue;
      var v = Number(row.dst);
      if (!isFinite(v)) continue;
      // SWPC times are UTC without trailing Z. Force UTC parse.
      var iso = String(row.time_tag || "");
    ...
    if (!series.length) throw new Error("swpc-dst: no values parsed");
    var latest = series[series.length - 1];
    return {
      dst: latest.v, symH: null, ae: null,
      when: new Date(latest.t).toISOString(),
