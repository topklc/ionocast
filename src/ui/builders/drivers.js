...
function refreshGeomagDrivers(paint) {
  Promise.allSettled([
    ...
    fetchData("dst").catch(function(){return null;}),
    ...
    var dstData = results[2].status === "fulfilled" ? results[2].value : null;
    ...
    if (dstData) {
      if (dstData.dst  != null) dst = (dstData.dst > 0 ? "+" : "") + Math.round(dstData.dst);
      if (dstData.symH != null) {
        symH = (dstData.symH > 0 ? "+" : "") + Math.round(dstData.symH);
        symHSub = "Dst <b>" + dst + "</b>";
      } else if (dstData.dst != null) {
