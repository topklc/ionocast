    ...
    function refresh() {
      jproxy(b.url).then(function(raw) {
        ...
        rows.forEach(function(d, idx) {
          var kp = +d.Kp;
          if (isNaN(kp)) return;
          // Absolute px against a 40px budget (not % of the column) so
          // the value label above and date label below can't squeeze
          // the bar; matches the outlook-kp chart in tables.js.
          var hPx = Math.max(kp / 9 * 40, 2);
          var bar = el("div", { className: "kp-bar", css: "height:" + hPx.toFixed(0) + "px;background:" + kpColor(kp) });
          var col = el("div", { className: "kp-col" });
          col.appendChild(el("span", { className: "kp-val", text: kp.toFixed(1) }));
          col.appendChild(bar);
