...
// Values reflect the current calibration ground truth: ducting recall
// 63 % / precision 71 % (GFS 2026-07-01 06z, post-moisture-gate).
...
// Decode the binary format via the shared codec (grid-codec.mjs owns
// the header layout; scripts/tests/tropo-codec.mjs round-trips it).
// This adapter just maps the codec's output onto the grid.json shape
// the renderer consumes.
function parseGridBinary(buffer) {
  const h = decodeGridBinary(buffer);
  const N = h.rows * h.cols;
  const vGridSrc = new Uint8Array(N);
  for (let i = 0; i < N; i++) {
    if (h.tropoIndex[i] === h.tropoIndex[i]) vGridSrc[i] = 1;
  }
  return {
    generated: new Date(h.genUnix * 1000).toISOString(),
    source: h.source,
    cycle: h.cycle,
    grid: h.grid,
    pressure_levels_hpa: h.levels,
    forecast_hour: h.forecastH,
    forecast_envelope: h.envelope,
    n_cells: N,
    n_valid: h.nValid,
    m_deficit_max: h.mDeficitMax,
    tropo_index_max: h.tropoIndexMax,
    cells: [],
    mGridSrc: h.tropoIndex,
    vGridSrc,
  };
}
...
export async function mountTropoMap(container) {
  ...
  if (typeof data.cycle === "string" && data.cycle.length >= 10
    ...
    if (m) {
      ...
      // Envelope grids (merge-leads max over two valid times) must not
      // present as a single instant; the flags bit set by the codec
      // lets us say so without parsing the source string.
      if (data.forecast_envelope) {
        validLabel = t("multi-lead envelope from {hh}:00 UTC {day} {month}", { hh, day, month }) + " · ";
      }
    }
  }
  meta.textContent =
