...
//                                              (custom cache dir)
//   TROPO_DEBUG_CELLS="38,-67.5;47.5,-87" node src/tropo/ingest.mjs
//                                              (print the vertical
//                                               profile + per-term
//                                               index breakdown for
//                                               the nearest cell to
...
// Moisture gate reference (hPa). A pure-temperature inversion only forms a
// real refractivity duct when there is enough near-surface water vapour for
// the wet term of N (3.73e5·e/T²) to drop across it. The temperature-only
// inversion bonuses are scaled by surface vapour pressure e, saturating to
// full strength at MOIST_E_REF. See the moisture-gate block in
// reduceMprofile for the physics. Env-tunable for calibration sweeps.
//
// Value chosen by a dry-land / duct-zone separation sweep over a live GFS
// cycle (no change to the canonical CA/Med/Gulf duct zones through 12, but
// California's ~14 hPa marine layer starts eroding by 20). 12 maximises
// separation while leaving every real duct zone intact, and matches the
// 15-25 hPa vapour pressure of genuine marine ducting air. Marine, evap, and
// M-deficit terms already carry humidity and are NOT gated.
//
// Confirmed against ground truth (GFS 20260701 06z, gated vs ungated on the
// same cycle): among the strongest ducting-flagged cells (tropo_index ≥ 100)
// the Antarctic share (≤70°S) drops from 60.3% → 1.0%, while the broad moist
// ducting field (≥10) is untouched (14.6% vs 15.0%). Sonde 3-class agreement
// is unchanged (5/10; ducting recall 63% / precision 71%) - the gate only
// trims dry-inversion false positives, no real duct crosses below the cut.
const MOIST_E_REF = parseFloat(process.env.TROPO_MOIST_E_REF || "12");  // hPa

// Cells to print a full vertical-profile + per-term breakdown for
// (calibration/diagnosis aid).  Format: "lat,lon;lat,lon;…".
const DEBUG_CELLS = (process.env.TROPO_DEBUG_CELLS || "")
  .split(";").filter(Boolean).map(s => {
    const [lat, lon] = s.split(",").map(Number);
    return { lat, lon };
  });

// Marine moisture-cliff reconstruction (see reduceMprofile).  K_CLIFF
// is M-units credited per hPa of integrated boundary-layer-to-
// subsidence vapour-pressure drop beyond CLIFF_MIN_DROP_HPA.
//
// Calibrated 2026-08-03 against Hepburn tropo-index maps
// (dxinfocentre.com) over 21 frames - 3 regions (eam/eur/wam) × 7
// valid times spanning 2 days of GFS 00z leads, 122k paired cells
// (scripts/hepburn-compare.py sweep, TROPO_EMIT_TERMS grids).  Mean
// Spearman vs Hepburn: K=0 → 0.59, K=4.5 → 0.746, K=5 → 0.748
// (better on 21/21 frames, never worse), plateau K=5-6, declining
// past 7.  K=5 with drop 8 is the strictly-dominant choice; the
// (K, drop) ridge trades off (higher K compensates higher drop) so
// re-sweep both together if either changes.
const K_CLIFF            = parseFloat(process.env.TROPO_K_CLIFF || "5");
const CLIFF_MIN_DROP_HPA = parseFloat(process.env.TROPO_CLIFF_MIN_DROP || "8");

// Nocturnal / stable moist land duct (see reduceMprofile).
const K_NOCT = parseFloat(process.env.TROPO_K_NOCT || "15");

// TROPO_EMIT_TERMS=1 adds the raw ingredients of the tunable terms to
// every cell (cliff_drop / cliff_scale / noct_raw) so a calibration
// sweep can recompose tropo_index for any K_CLIFF / CLIFF_MIN_DROP /
// K_NOCT offline without re-running the GRIB extraction:
//   cliff(K, D) = min(70, K·max(0, cliff_drop − D)) · cliff_scale
//   noct(K)     = min(40, K·noct_raw)               · (1 − windPenalty)
// where cliff_scale already folds in the height weight, resolved-duct
// fade, convective halving, and wind penalty.
const EMIT_TERMS = process.env.TROPO_EMIT_TERMS === "1";

// Thirteen pressure levels chosen to give dense resolution where ducting
...
function extractField(gribPath, whereClauses, latStep, lonStep) {
  ...
  // of transient heap).
  // Output grid ON the native GFS lattice (multiples of latStep from
  // the poles), poles included. The previous grid put cell centers at
  // half-step offsets (-90 + step/2 …), which at 0.25° placed every
  // GFS input latitude exactly halfway between two output centers:
  // Math.round's half-toward-+∞ tie-break then shifted the whole map
  // ~14 km north of its labeled centers, silently dropped the -90°
  // row, and only stream order (north→south, last-write-wins) masked
  // the same misregistration at 0.5°. On the native lattice every
  // cell has an exact-match input sample and the residual filter
  // below makes registration independent of stream order.
  const latMin = -90;
  const latMax = 90;
  const lonMin = -180;
  ...
  for (const line of r.stdout.split("\n")) {
    ...
    if (L >= 180) L -= 360;
    if (L < -180) L += 360;
    // Exact-lattice registration: accept a sample only when it sits on
    // (or within a quarter-step of) an output node. When the output is
    // coarser than the 0.25° input (e.g. TROPO_RES=0.5) this skips the
    // in-between input rows instead of letting round()'s tie-break
    // assign them to a neighbouring cell.
    const rowF = (latMax - lat) / latStep;
    const colF = (L - lonMin) / lonStep;
    const row = Math.round(rowF);
    let col = Math.round(colF);
    if (Math.abs(rowF - row) > 0.25 || Math.abs(colF - col) > 0.25) continue;
    // Date-line wrap: input lon in (180 − lonStep, 180) belongs to the
    // -180 column, not off-grid (the old code discarded it, leaving a
    // NaN seam at the antimeridian for sub-step-offset longitudes).
    if (col === COLS) col = 0;
...
function profileM(levels) {
  ...
  // semi-enclosed seas).
  // eMbar is carried through for the moisture-direction checks in
  // reduceMprofile (elevated marine moisture cliff, fog gate).
  return levels.map(L => {
    const N = refractivity(L.tK, L.pMbar, L.eMbar);
    return { hM: L.hM, M: N + 0.157 * L.hM, tK: L.tK, eMbar: L.eMbar };
  });
}
...
function reduceMprofile(mProfile, surfVars, orogM) {
  ...
  // no full duct exists).  Per-layer contribution is modest (11-51
  // M-units) but accumulates over 2-3 super-refractive layers in
  // a typical marine column.
  let superRefractiveSum = 0;

  // Minimum thickness for *gradient-band* tests (the SR sum and the
  // convection check divide by dh). The synthesized ground level sits
  // at orog + 2 m; when surface pressure is only fractionally above a
  // member of LEVELS_HPA the first layer is metres thin, and over such
  // a layer Float32/print-precision noise reads as a huge gradient:
  // ±0.004 M-units over 5 m is -156 N/km (nearly the whole ducting
  // budget from pure noise), 0.1 K over 8 m is -12.5 K/km (spuriously
  // arming the convective suppression). 30 m mirrors the spirit of
  // deltaNFromRows' 100 m guard, scaled to GFS's coarser vertical
  // noise floor. Difference-based terms (duct intensity, inversions)
  // are bounded by actual ΔM/ΔT and stay un-guarded.
  const MIN_GRADIENT_LAYER_M = 30;

  // Top of the span the surface-inversion bonus (below) measures:
  // ground+2 m up to the first level > 200 m AGL (typically ~320 m
  // over ocean, spanning the first TWO layers of the profile). Layer
  // inversions entirely inside that span skip the layer term so one
  // physical inversion isn't billed by both terms - and then a third
  // time by the HPBL bonus the pair could jointly arm.
  let surfBonusTopHm = null;
  if (surfVars && surfVars.T2m_K != null) {
    const upTop = mProfile.find(l => l.hM > groundHm + 2 + 200);
    if (upTop) surfBonusTopHm = upTop.hM;
  }
  ...
  for (let i = 0; i < mProfile.length - 1; i++) {
    ...
    if (hAGL < 3000 && dh >= MIN_GRADIENT_LAYER_M) {
      const dN_per_km = ((actualDM / dh) - 0.157) * 1000;
      if (dN_per_km < -79 && dN_per_km > -157) {
        // Thin-layer taper: full contribution above 150 m thickness
        // (typical marine SR layers are 200-400 m, so the calibrated
        // carpet is untouched); sub-150 m layers contribute
        // proportionally so a 40 m sliver can't buy a full layer's
        // worth of index from a marginal gradient.
        superRefractiveSum += (-79 - dN_per_km) * Math.min(1, dh / 150) * w;
      }
    }

    // Convection check (super-adiabatic in lowest 2 km AGL).  Over
    // ocean the surface-adjacent layer is exempt: a warm sea keeps
    // the lowest ~100 m marginally super-adiabatic almost everywhere
    // (SST > T_air by a fraction of a kelvin is enough), and that
    // shallow mixing coexists with - rather than destroys - the
    // trade/subsidence inversion aloft.  Counting it as "convective"
    // halved the duct terms across exactly the marine columns the
    // moisture-cliff term below is designed to score.
    if (hAGL < 2000 && dh >= MIN_GRADIENT_LAYER_M &&
        actualDT / dh < -DRY_ADIABATIC_K_PER_M &&
        !(surfVars && surfVars.isOcean && hAGL < 100)) {
    ...
    if (i > 0 && (surfBonusTopHm == null || next.hM > surfBonusTopHm)) {
      // Expected-lapse depth capped at 500 m: the 3 K threshold is an
      // absolute-kelvin cut, and over an uncapped thick layer a merely
      // *isothermal* (stable, non-inverted) column trips it once
      // dh > ~460 m - routine for the 50-hPa-spaced layers above
      // 850 hPa (dh 450-850 m). Layers that actually score in practice
      // sit below 850 hPa with dh ≈ 200-450 m and are unaffected, so
      // the calibrated cuts hold; the cap only removes the free
      // thickness-credit that let deep stable (not inverted) layers
      // masquerade as inversions.
      const expectedDT = -STD_LAPSE_K_PER_M * Math.min(dh, 500);
  ...
  if (surfVars && surfVars.T2m_K != null) {
    ...
    if (upperLevel) {
      // Same expected-lapse depth cap as the layer loop (600 m here:
      // the target level is "first > 200 m AGL", typically ~320 m over
      // ocean - unaffected - but over high terrain it can sit 1-2 km
      // up, where the uncapped expected drop of 6.5-13 K made ANY
      // stable profile read as a multi-kelvin surface "inversion" at
      // 10 M-units per kelvin).
      const dh = upperLevel.hM - t2mHm;
      const expectedT = surfVars.T2m_K - STD_LAPSE_K_PER_M * Math.min(dh, 600);
      const inversionK = upperLevel.tK - expectedT;
      // This bonus exists to credit SUB-GRID surface inversions - ones
      // T_2m sees but the pressure levels smooth over.  When the level
      // profile itself already resolves the inversion (level-to-level
      // T rise across the same span), the M-profile terms (SR sum /
      // duct intensity) have already measured its real refractive
      // effect, wet term included; re-billing it here painted cold
      // lakes and coastal upwelling zones (air warm aloft, resolved
      // 8-10 K inversions, sub-refractive moisture) at full ducting
      // strength - cells Hepburn correctly shows as below normal.
      // Credit only the unresolved remainder.
      let resolvedK = 0;
      const spanBase = mProfile.find(l => l.hM >= t2mHm - 5);
      if (spanBase && upperLevel.hM > spanBase.hM) {
        const rdh = upperLevel.hM - spanBase.hM;
        const rexp = spanBase.tK - STD_LAPSE_K_PER_M * Math.min(rdh, 600);
        resolvedK = Math.max(0, upperLevel.tK - rexp);
      }
      const unresolvedK = Math.max(0, inversionK - resolvedK);
      ...
      if (unresolvedK > 1) {
        surfaceInversionBonus = Math.min(100, 10 * (unresolvedK - 1));
      }
    }
  }
  ...
  // ── Moisture gate on the temperature-only inversion terms ───────
  // surfaceInversionBonus and layerInversionSum are measured in kelvin of
  // temperature inversion, with no humidity dependence. But ducting
  // refractivity is dominated by the wet term of N (3.73e5·e/T²): a warm
  // marine inversion with e≈15 hPa traps strongly, while an Antarctic-
  // winter radiative inversion of the SAME ΔT but e<1 hPa barely bends a
  // signal. Left ungated, the dry-but-intense polar inversions saturate the
  // surface bonus and paint the winter pole as solid ducting (over half of
  // all globally-flagged ducting cells were poleward of 70°S). Scale both
  // temperature-inversion terms by surface vapour pressure, ramping linearly
  // to full strength at MOIST_E_REF. Applied BEFORE the HPBL bonus so a dry
  // inversion can't trip the "confirmed inversion" gate there. The M-based
  // duct intensity, marine, and evaporation terms already carry humidity, so
  // they are NOT gated.
  let moistGate = 1;
  if (surfVars && surfVars.D2m_K != null) {
    const eAct2m = eSat(surfVars.D2m_K - 273.15);  // dewpoint sat = actual e
    moistGate = Math.max(0, Math.min(1, eAct2m / MOIST_E_REF));
  }
  surfaceInversionBonus *= moistGate;
  layerInversionSum     *= moistGate;
  ...
  // ── Marine moisture-cliff reconstruction ────────────────────
  // The trade/subsidence inversion over subtropical oceans (Azores/
  // Bermuda high, California/Baja, Canary, ...) is a sharp ~100-200 m
  // humidity cliff at 800-1500 m that GFS's ~250-500 m level spacing
  // smears across 3-4 layers, dropping every per-layer dN/dh into or
  // below the -79 N/km super-refractive onset even when the real
  // inversion ducts hard.  The column-integrated moisture drop
  // survives the smearing: e falls from 20-27 hPa in the marine BL to
  // < 45 % of that under the subsidence aloft.  Reconstruct the duct
  // credit from that integrated drop.  Scaled down when the profile
  // already resolves a duct (no reconstruction needed - same
  // anti-double-count logic as the surface-bonus resolvedK check).
  let moistureCliffBonus = 0, cliffDrop = 0, cliffScale = 0;
  if (surfVars && surfVars.isOcean) {
    let eLow = 0, eLowHm = null;
    for (const l of mProfile) {
      if (l.hM - groundHm > 500) break;
      if (l.eMbar > eLow) { eLow = l.eMbar; eLowHm = l.hM; }
    }
    let eHi = Infinity, cliffHm = null;
    for (const l of mProfile) {
      const hAGL = l.hM - groundHm;
      if (hAGL > 3000) break;
      if (eLowHm != null && l.hM > eLowHm) {
        if (l.eMbar < eHi) eHi = l.eMbar;
        // Cliff height: where e first falls through 60 % of the BL value.
        if (cliffHm == null && l.eMbar <= 0.6 * eLow) cliffHm = l.hM;
      }
    }
    if (eLow > 0 && isFinite(eHi) && eHi < 0.45 * eLow && cliffHm != null) {
      const cliffAGL = cliffHm - groundHm;
      const w = cliffAGL <= 2000 ? 1.0 : (cliffAGL <= 3000 ? 0.5 : 0);
      cliffDrop = eLow - eHi;
      // Resolved-duct fade: a profile that already shows a >= 20
      // M-unit duct needs no reconstruction.  Convective columns get
      // the same halving as the other duct terms.
      cliffScale = w * Math.max(0, 1 - maxDuctIntensity / 20)
                     * (lowAltConvective ? 0.5 : 1);
      moistureCliffBonus =
        Math.min(70, K_CLIFF * Math.max(0, cliffDrop - CLIFF_MIN_DROP_HPA))
        * cliffScale;
    }
  }

  // ── Nocturnal / stable moist land duct ──────────────────────
  // Shallow radiation inversions over moist land (Gulf coast plains,
  // Po valley, ...) are entirely sub-grid: the strongest gradient
  // lives in the lowest 50-150 m and every GFS layer straddles it.
  // GFS's own boundary-layer scheme still diagnoses the regime: a
  // collapsed HPBL (< 400 m over land = stable BL, i.e. the surface
  // has decoupled), calm 10 m wind, and rich 2 m moisture are jointly
  // the fingerprint.  Credit scales with the moisture available to
  // drop across the unresolved inversion and fades linearly with
  // wind and BL depth.
  let noctBonus = 0, noctRaw = 0;
  if (surfVars && !surfVars.isOcean
      && surfVars.D2m_K != null
      && surfVars.hpbl_m != null && surfVars.hpbl_m < 400
      && surfVars.wind10m_ms != null && surfVars.wind10m_ms < 4) {
    const eAct2m = eSat(surfVars.D2m_K - 273.15);
    if (eAct2m > 15) {
      noctRaw = (eAct2m - 15)
              * (1 - surfVars.wind10m_ms / 4)
              * ((400 - surfVars.hpbl_m) / 400);
      noctBonus = Math.min(40, K_NOCT * noctRaw);
    }
  }
  ...
  // M-units, landing well inside the ducting segment of the renderer's
  // two-segment cut ramp (CUT_DUCTING = 90 up to CUT_MAX = 200), deep
  // in the warm bands.  superRefractiveSum builds the broad marine
  ...
  return {
    ...
    // Per-term breakdown for TROPO_DEBUG_CELLS and calibration sweeps.
    terms: {
      duct5x:      5 * maxDuctIntensity,
      superRefr:   superRefractiveSum,
      layerInv:    layerInversionSum,
      surfInv:     surfaceInversionBonus,
      marineInv:   marineInversionBonus,
      hpbl:        hpblBonus,
      evap:        evapBonus,
      cliff:       moistureCliffBonus,
      noct:        noctBonus,
      cliffDrop, cliffScale, noctRaw,
      windPenalty, moistGate, lowAltConvective,
    },
  };
}
...
async function main() {
  ...
  for (let r = 0; r < ROWS; r++) {
    for (let c = 0; c < COLS; c++) {
      ...
      for (const Lhpa of LEVELS_HPA) {
        ...
        // Fallback below-ground filter when SP is missing: surfaceHpa
        // degrades to Infinity above, which admits *every* level -
        // including the below-ground phantoms this filter exists to
        // reject. Geopotential height under the orography is the same
        // physical test expressed in metres.
        if (!isFinite(spPa) && isFinite(orog) && H < orog) continue;
      ...
      const { m_deficit, tropo_index, surface_duct, terms } = reduceMprofile(
        mProfile, surfVars, isFinite(orog) ? orog : null);
      const dbg = DEBUG_CELLS.find(d =>
        Math.abs(d.lat - lat) <= latStep / 2 && Math.abs(d.lon - lon) <= lonStep / 2);
      if (dbg) {
        console.log(`\n── debug cell ${lat},${lon} ──`);
        console.log(`  surfVars: ${JSON.stringify(surfVars)}`);
        console.log(`  orog ${isFinite(orog) ? orog.toFixed(0) : "n/a"} m`);
        for (const l of mProfile) {
          console.log(`    h ${l.hM.toFixed(0).padStart(5)} m  T ${(l.tK - 273.15).toFixed(1).padStart(5)} °C  e ${l.eMbar.toFixed(1).padStart(5)} hPa  M ${l.M.toFixed(1)}`);
        }
        console.log(`  terms: ${JSON.stringify(terms)}`);
        console.log(`  tropo_index ${tropo_index.toFixed(1)}  m_deficit ${m_deficit.toFixed(1)}`);
      }
      const cell = {
        lat, lon,
      ...
      if (EMIT_TERMS) {
        // Compact keys - the sweep JSON holds 260k cells.
        cell.tw = Number((1 - terms.windPenalty).toFixed(3));
        if (terms.cliffDrop)  cell.cd = Number(terms.cliffDrop.toFixed(2));
        if (terms.cliffScale) cell.cs = Number(terms.cliffScale.toFixed(3));
        if (terms.noctRaw)    cell.nr = Number(terms.noctRaw.toFixed(3));
      }
      cells.push(cell);
    }
  }
  ...
  const out = {
    ...
    source:       `NOAA GFS ${RESOLUTION}° (NOMADS subset, +${LEAD_HOURS}h)`,
    cycle:        `${yyyymmdd}${hh}z`,
    grid: {
