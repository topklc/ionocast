...
import {
  ...
  hopsForDistance, mufObliqueFactor,
...
export function deriveConditions(ctx) {
  ...
  function fmuf(n) {
    ...
    // Column 4 is a dimensionless f/MUF ratio (formatted with toFixed(2)),
    // not a MHz value. Normalize locale decimal commas to dots so
    // European-locale renderers that emit "1,42" parse correctly. The
    // previous parseFloat saw "1,42" and stopped at the comma, returning
    // 1 silently.
  ...
  (function buildHubCells() {
    ...
    HUB_ANCHORS.forEach(function (a) {
      ...
      for (var i = 0; i < gp.length; i++) {
        ...
        // Local NVIS ring cells are excluded from hub snapping: a hub
        // that happens to sit near the QTH should keep its worldwide-
        // reachability semantics (F2 grid cell), not silently become
        // an NVIS margin in the hub mean.
        if (p.local) continue;
  ...
  //   recovery  - Dst depressed (<= -30) but kpEffective > kpNow + 0.5
  //               (Kp has dropped while the lagged F-region remains
  //               elevated; TID tail). MUST be tested before main: Dst
  //               recovers over days, so a deep storm still reads
  //               Dst <= -50 long after Kp has fallen, and the old
  //               main-first ordering (whose kpEffective >= kpNow
  //               condition is a superset of recovery's whenever
  //               Dst <= -50) fired the +4 dB main amplification
  //               during the physical recovery and made the recovery
  //               TID sigma unreachable for deep storms.
  //   main      - Dst <= -50 with Kp still driving (not yet into the
  //               recovery signature above). Covers both the rising
  //               edge (kernel still loading, kpEffective < kpNow --
  //               the period of maximal oval expansion, which the old
  //               ordering misfiled as "active") and the matured peak.
  ...
  var stormPhase = (function() {
    ...
    if (dstNow != null && dstNow <= -50) return "main";
  ...
  function hfGroupVerdict(names) {
    ...
    function tryMargin(name, pathCtx) {
      ...
      // Skip-zone guard: below 1500 km the F2-hop budget mis-prices the
      // path as a trivial low-loss hop and compares it against a
      // MUF(3000)-referenced consensus that a short path cannot use.
      // A local path that the NVIS gate rejects (band above the NVIS
      // MUF, or no local digisonde reading) sits in the skip zone -
      // structurally closed, not "open via F2". Only the local ring
      // cells (paths.js) have dKm < 1500; the 10° grid starts at
      // GRID_MIN_KM = 1500 and is unaffected.
      if (dKm != null && dKm < 1500 && !nvisAvailable) return null;
      var f2Muf    = pathCtx.mufMHz != null ? pathCtx.mufMHz : null;
      var isNvis = nvisTail >= 0.5;        // mode label only

      // Hop-length MUF re-referencing. kc2g mufd and the climatology
      // leg (foF2 × 3.0) are both MUF(3000) products; the frequency a
      // real hop supports scales with M(d_hop)/M(3000). For ceiling-
      // range DX hops the ratio is ~1.0-1.03 (the calibrated regime is
      // untouched); for 1500-2500 km single hops it is ~0.73-0.9,
      // trimming the up-to-30% MUF overstatement that pushed 10/12/15 m
      // toward false-open on exactly the short cells most likely to win
      // "best path". The whole consensus/pathMinMuf pipeline below runs
      // 3000-referenced (nvisMuf, the only distance-referenced input,
      // is divided by mObl on the way in - exact round-trip, since
      // mufObliqueFactor reproduces the NVIS secant at short range) and
      // effMuf is re-referenced to the actual hop once at the end.
      var mObl = 1;
      if (dKm != null && dKm > 0) {
        var hObl = hFLive != null ? hFLive : 300;
        mObl = mufObliqueFactor(dKm, hObl) / mufObliqueFactor(3000, hObl);
      }

      var kc2gMuf;
      if (nvisAvailable && nvisTail > 0 && nvisMuf != null && f2Muf != null) {
        kc2gMuf = nvisTail * (nvisMuf / mObl) + (1 - nvisTail) * f2Muf;
      } else if (nvisAvailable && nvisTail > 0 && nvisMuf != null) {
        kc2gMuf = nvisMuf / mObl;
      } else {
        kc2gMuf = f2Muf;
      }
      ...
      var climoMuf = null;
      // Pure-climatology midpoint MUF, kept SEPARATE from the second
      // opinion above. pathMinMuf re-prices each reflection point as
      //   muf_k = 3·foF2Climatology(p_k) · (mufMid / climoMufMid)
      // which is only a shape correction - muf_k = mufMid ·
      // climo(p_k)/climo(mid), the paper's Eq. pathmuf - if the
      // denominator is the same climatology the numerator samples.
      // Passing the *fused* midpoint value (as this did from the
      // 2026-05-13 FUSE_PRIMARY_FOF2 flip until 2026-08-01) turned the
      // scalar into climo(mid)/fuse(mid), a factor with no physical
      // meaning, and because pathMinMuf seeds its running minimum at
      // mufMidpoint the error was one-sided: fuse < climo was clipped
      // away, fuse > climo depressed the path MUF. On odd hop counts the
      // midpoint IS a reflection point, so the model discarded MUF at the
      // very point carrying the observation - up to ~19 dB of phantom
      // over-MUF loss on 20 m for a 6.0 → 9.0 MHz climo/fuse gap.
      // The observations still reach the budget, through mufMid.
      var climoMufPure = null;
      if (pathCtx.midLat != null && pathCtx.midLon != null) {
        // Pure climatology at the midpoint, computed first because it is
        // both the pathMinMuf denominator and the third branch's second
        // opinion - one evaluation serves both.
        var foF2pure = cosZmid != null
          ? foF2Climatology(
              f107A, cosZmid, Math.abs(pathCtx.midLat),
              pathCtx.midLat, pathCtx.midLon, nowDate
            )
          : null;
        if (foF2pure != null) climoMufPure = foF2pure * 3.0;
        ...
        if (FUSE_PRIMARY_FOF2 && fuseGrid) {
          ...
          // Third branch: the two are the same number by construction,
          // which is why this split is a no-op when no fused second
          // opinion is in play.
          foF2c = foF2pure;
        }
      ...
      // reflection point, scaled by the consensus/climatology correction
      // at the midpoint. climoMufPure may be null (no midpoint geometry),
      // pathMinMuf falls back to illumination-ratio scaling in that case.
      if (pathCtx.midLat != null && pathCtx.destLat != null && pathCtx.dKm) {
        effMuf = pathMinMuf(effMuf, climoMufPure, f107A,
                            pathCtx.midLat, pathCtx.midLon,
                            ll[0], ll[1],
                            pathCtx.destLat, pathCtx.destLon,
                            pathCtx.dKm, nowDate, hFLive);
      }

      // Re-reference the 3000-referenced consensus to the actual hop
      // length (see mObl above). Applied after pathMinMuf so its
      // internal foF2 × 3.0 per-hop sampling stays in the same
      // reference frame as the climoMuf argument.
      effMuf = effMuf * mObl;

      // Path-midpoint foEs, resolved once and used by BOTH consumers on
      // this path: lEsScreenDb inside the F2 budget (signals punching
      // *through* the E layer on their way to F2) and snrMarginHfEs
      // below (signals reflecting *off* it). Both describe the same
      // layer at the same reflection point, so they must read the same
      // sample. Until 2026-08-01 the F2 leg took bandOpts' default
      // `foEs: foes` - a single value scraped off the VHF band table,
      // i.e. the operator's local Es - so an 8000 km path was screened
      // on the Es over the QTH while the Es-mode leg one branch below
      // correctly used the midpoint grid.
      var foesPath = foes;
      ...
      if (SCATTER_WEIGHT > 0 && pathCtx.dKm && giroStations.length >= 2 &&
        ...
        var nHops = hopsForDistance(pathCtx.dKm, hFLive);
        if (nHops >= 2) {
      ...
      // foesPath (resolved above, before the F2 budget) is the
      // per-midpoint foEs from the fuse Es grid, falling back to the QTH
      // station value when no grid observation is in range. The grid
      // covers the path's reflection point, where the Es layer actually
      // intercepts the signal, which on long paths can be very different
      // from the QTH foEs.
      if (foesPath != null && foesPath > 0) {
        ...
        // Mode-selection symmetry: gray-line is a D-region refund and
        // the Es signal transits the D region too (snrMarginHfEs
        // charges lAbsD at the midpoint), so the Es leg earns the same
        // credit before the comparison. Without this the F2 margin
        // entered the contest bonus-assisted and a genuinely louder Es
        // opening near the terminator lost the mode label (and its
        // margin was understated when it did win). TEP and scatter
        // stay F2-only - they are F-region irregularity mechanisms
        // with no Es analogue.
        if (mEs != null && glBonus > 0) mEs.margin += glBonus;
        if (mEs != null && mEs.margin > m.margin) {
          ...
          // in the Es budget now; lHop, lLow are Es-irrelevant so
          // zero-fill; lIono carries L_IONO_ES_DB so per-mechanism
          // breakdown consumers don't drift by 15 dB on Es rows).
          m = {
            margin: mEs.margin, sigma: mEs.sigma,
            lFs: mEs.lFs, lAbs: mEs.lAbs, lAbsD: mEs.lAbsD,
            lAur: mEs.lAur, lMuf: mEs.lMuf,
            lLow: 0, lHop: 0, lEs: 0, lIono: mEs.lIono,
    ...
    // Cells with no fresh digisonde within range arrive with
    // mufMHz == null and are scored anyway: tryMargin's consensus falls
    // back to the fuse grid / climatology for the kc2g-less leg
    // (mufConsensus handles k == null natively). The previous
    // `p.mufMHz == null` skip made every station-less ocean and
    // southern-hemisphere sector invisible to the verdict - biasing
    // openDirs/breadth toward digisonde-dense NA/EU/JP and letting hub
    // anchors snap to never-scored cells - even though the global fuse
    // grid exists precisely to cover those cells.
    pathList.forEach(function(p) {
      if (!p.lengthKm) return;
      var ctx = {
    ...
    if (best) {
      ...
      if (absorbed && spots < absorbedThreshold) {
        // Sync the per-band row like the spot-override branch does:
        // without this the group verdict read "closed" while the HF
        // table's bestPerBand row kept the physics tier (possibly
        // "good"), showing the operator two contradictory answers.
        if (best && bestPerBand[best.name]) {
          bestPerBand[best.name].tier = "closed";
        }
        return ["closed", t("D-region absorption blocking signals"), bestPerBand];
      }
      ...
      var avg = absorbedBaseline;  // same 30-day mean the D-RAP gate read
  ...
  return {
    ...
    concurrent: {
      ...
      //   quiet     - Dst > -30 and Kp < 4
      //   initial   - Dst > 0 with elevated Kp (sudden compression, rare)
      //   recovery  - Dst depressed but kpEffective > kpNow + 0.5 (Kp
      //               down, lagged F-region still elevated; tested first)
      //   main      - Dst <= -50 with Kp still driving (rising edge or
      //               matured peak, not yet the recovery signature)
      stormPhase: stormPhase,
