...
export function computeFuseEsGrid(ctx, opts) {
  ...
  return buildFoF2Grid({
    ...
    // 2.5°, not the foF2 grid's 10°: the giro_foes localization is
    // 500 km, and sampling a 500-km-coherence field on ~1100 km grid
    // nodes aliased Es patches away - a strong reading could sit
    // between four nodes and be diluted toward the 1 MHz floor before
    // the bilinear lookup ever saw it. 2.5° (~275 km) resolves the
    // localization kernel. Cost is fine: the Es observation list is a
    // few dozen stations, so the build is ~10^5 kernel evaluations.
    resolutionDeg: opts.resolutionDeg || 2.5,
    latRange:      opts.latRange,
    sources:       SOURCES,
  });
}
