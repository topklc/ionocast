...
export function foF2Climatology(f107A, cosZ, latAbs, lat, lon, date) {
  ...
  // Winter anomaly: midlatitude (30° to 60°, matching the gate below)
  // daytime foF2 runs ~10 to 15%
...
//
// Independence caveat (2026-08-01). The symmetric-blend argument above
// was written when the second leg was pure P.1239 climatology, i.e.
// genuinely independent of kc2g. Since FUSE_PRIMARY_FOF2, the caller
// passes the fuse-grid value, whose dominant observation source is the
// GIRO station set - and kc2g's mufd is itself a digisonde-assimilated
// map. The two legs now share their measurements, so `divergence`
// under-reports true disagreement (it measures mostly interpolation and
// prior-weight differences, not two independent opinions), and the
// geometric mean damps less than it appears to. Nothing here is wrong,
// but do not read `divergence` as a confidence signal without
// re-deriving what it means against the current source graph.
...
// read 6 to 10 MHz. (The illumination-ratio form surviving below as
// the no-climatology fallback carries a max(F, ·) night floor -
// nightFloor(), 0.20–0.60 - so its night reading is F·MUF_mid, not
// 0.224·MUF_mid; the 0.224 figure describes the pre-floor original.)
// The new form evaluates foF2Climatology directly at
...
//
// CALLER CONTRACT (climoMufMidpoint). This function samples
// foF2Climatology at each reflection point and rescales by
// mufMidpoint / climoMufMidpoint. That scalar is a *shape* correction
// and is only meaningful when climoMufMidpoint is 3.0 × the SAME
// foF2Climatology evaluated at (midLat, midLon) - then the ratio
// collapses to muf_k = mufMidpoint · climo(p_k)/climo(mid), which is
// the documented behaviour, and the midpoint hop reproduces
// mufMidpoint exactly. Do NOT pass a fused / observed / otherwise
// corrected midpoint MUF here: mufMidpoint is where the observation
// signal belongs, and mixing sources makes the scalar an arbitrary
// climo/observation ratio. Because the running minimum is seeded at
// mufMidpoint, such a mismatch can only ever depress the result -
// it is silently one-sided, which is why it survived from the
// 2026-05-13 FUSE_PRIMARY_FOF2 flip to the 2026-08-01 audit.
// (Sampling the fuse grid per hop instead of climatology would be a
// strict information gain and is tracked in BACKLOG.md; it is a
// different change and needs its own harness pass, because the fuse
// grid's 10° nodes smooth the terminator gradient this term exists
// to resolve.)
export function pathMinMuf(mufMidpoint, climoMufMidpoint, f107A,
  ...
  // Optional live hF (typically giroHmF2) keeps the hop count here in
  // lockstep with the SNR budget's hopsForDistance(dKm, hF): without
  // it, MUF sampling and loss sampling disagreed about hop geometry on
  // elevated/depressed hmF2 days.
  var n = hopsForDistance(dKm, hF);
  ...
  if (climoMufMidpoint == null || climoMufMidpoint <= 0 || f107A == null) {
    // 2026-07-10 cleanup: the floor was a hard-coded 0.4 literal while
    // nightFloor() above sat exported-but-uncalled - the documented
    // contradiction in the paper's night-floor section. nightFloor()
    // returns exactly 0.4 when f107A is null (the unit-test path, so
    // that behaviour is unchanged); when f107A is present but the
    // climatology midpoint is unavailable, the floor now scales with
    // solar activity and per-point latitude as originally intended.
    var cosZmid0 = solarCosZenith(midLat, midLon, date);
    var shapeMid0 = _zenithShape(cosZmid0, nightFloor(f107A, Math.abs(midLat)));
    ...
    for (var i = 1; i <= n; i++) {
      ...
      var muf_i = mufMidpoint *
        _zenithShape(cz, nightFloor(f107A, Math.abs(p[0]))) / shapeMid0;
