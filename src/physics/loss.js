...
import {
  ...
  PCA_FLUX_REF_PFU, PCA_CGM_THRESHOLD,
...
import { hopCeilingKm, solarCosZenith, cgmLatAbs } from "./geometry.js";
...
// form under-predicted on. cosZ <= 0 returns 0 (night); the [0, 0.05]
// twilight sliver is linearly ramped rather than hard-gated (2026-08-01,
// see the ramp note in the body).
//
...
export function lAbsDiurnalDb(fMHz, cosZ) {
  if (cosZ == null || isNaN(cosZ) || cosZ <= 0) return 0;
  if (fMHz == null || !isFinite(fMHz) || fMHz <= 0) return 0;
  var dayLoss = D_REGION_PREFACTOR / Math.pow(fMHz + 0.5, 2);
  // Smooth twilight ramp over cosZ in [0, 0.05], identical in form and
  // interval to the one lFlareDb applies below, and for the same reason.
  // The bare `cosZ < 0.05 return 0` gate this replaces was not a night
  // gate but a cliff: cos^0.7 at the cut is 0.123, not 0, so the term
  // stepped 0 -> 4.45 dB on 160 m (1.43 on 80 m, 0.43 on 40 m) the
  // instant the sun cleared ~2.9 deg of elevation at a reflection point.
  // That is the same cliff family retired from the auroral Kp gate, the
  // CGM edge, the hop boundary, the Es screen and the flare twilight;
  // it fired twice per day on every path, on the two bands where the
  // term is largest and where the terminator-peaked grayline credit is
  // simultaneously at its maximum, so the two stepped against each other.
  // Values at cosZ >= 0.05 are unchanged, so the sunlit regime the
  // calibration basket was fit against does not move; only the ~20 min
  // twilight sliver either side of local sunrise/sunset changes, from a
  // hard zero to a linear approach to zero.
  var ramp = cosZ >= 0.05 ? 1 : (cosZ / 0.05);
  return dayLoss * Math.pow(cosZ, 0.7) * ramp;
}
...
// u = ε_c − cos²ψ
// R_h = (sin ψ − √u) / (sin ψ + √u)
// R_v = (ε_c·sin ψ − √u) / (ε_c·sin ψ + √u)
// Loss = −10 · log10(|R|²), clamped to [0, 8] dB. The clamp guards
// against unrealistic blow-up from the single-layer earth simplification
// (and, for V-pol, bounds the pseudo-Brewster notch, where the
// single-layer model is at its least trustworthy anyway).
//
// Polarisation selection (2026-07-03): callers with antType "vertical"
// get the R_v branch. This is NOT the ruled-out AVG experiment above -
// that applied a polarisation blend to *every* user unconditionally and
// lost the calibration because the population is overwhelmingly
// H-pol-launched. By the same stay-mostly-launched-pol argument that
// won the calibration for H-pol, a vertical launches V-pol and sees
// V-pol Fresnel at each bounce - several dB more per-hop loss near the
// pseudo-Brewster angle. Charging a vertical user the H-pol branch was
// specifically wrong for exactly them.
export function lHopGroundReflectionDb(fMHz, elevDeg, antType) {
  if (!isFinite(fMHz) || fMHz <= 0) return 0;
  ...
  // Numerator / denominator share structure across both branches:
  // H-pol uses a = sinψ (real); V-pol uses a = ε_c·sinψ (complex).
  var aRe = sinP, aIm = 0;
  if (antType === "vertical") {
    aRe = eRe * sinP;
    aIm = eIm * sinP;
  }
  var nRe = aRe - sqRe, nIm = aIm - sqIm;
  var dRe = aRe + sqRe, dIm = aIm + sqIm;
...
export function lMultiHopDb(nHopsOrDKm, fMHz, elevDeg, hF, antType) {
  if (nHopsOrDKm == null || !isFinite(nHopsOrDKm)) return 0;
  var perHop = lHopGroundReflectionDb(fMHz, elevDeg, antType) + DEFOCUS_DB_PER_EXTRA_HOP;
...
// peak extra loss, ramped smoothly on both edges (2026-07-03): the
// prior hard gates (foEs ≥ 5 MHz AND f < 2·foEs) were the last
// surviving members of the retired cliff family - foEs ticking
// 4.99 → 5.00 stepped every band below 2·foEs by 5 dB at once, and a
// band sitting exactly at f = 2·foEs flipped the full charge on a
// hundredth-of-a-MHz foEs drift. Now: intensity factor ramps 0 → 1
// over foEs 4 → 5 MHz, and the screening factor ramps 1 → 0 over
// f = 1.6·foEs → 2·foEs (partial screening near the penetration edge).
// Values at the old anchor points are unchanged (full 5 dB for a
// strong layer well below penetration; 0 beyond it).
export function lEsScreenDb(fMHz, foEs) {
  if (foEs == null || !isFinite(foEs)) return 0;
  var intensity = foEs - 4;                       // 0 at 4 MHz → 1 at 5 MHz
  if (intensity <= 0) return 0;
  if (intensity > 1) intensity = 1;
  var screen = (2 * foEs - fMHz) / (0.4 * foEs);  // 1 at 1.6·foEs → 0 at 2·foEs
  if (screen <= 0) return 0;
  if (screen > 1) screen = 1;
  return 5 * intensity * screen;
}
...
// for hours to days. Gate: GOES >=10 MeV integral proton flux elevated
// above the ~1 pfu quiet-time background AND path traverses the polar
// cap (|CGM| > 60°). Magnitude scales with log10(flux / 1 pfu) and
// f^-1.0 (PCA's empirical frequency dependence per NOAA D-RAP and
// Sauer & Wilkinson 2008; the classical Castelli & Aarons f^-1.5 fits
// chromospheric flare D-region absorption better and over-attenuates
// PCA at HF). Per-hop; pathIonoLosses sums across reflection points.
//
...
// same conditions. Current form (driver = 9, f^-1.0, referenced to the
// 1 pfu quiet background) yields, at the NOAA S-scale thresholds:
//   S1 (10 pfu):    9 × 1 × (7/14) = 4.5 dB at 14 MHz
//                                    9 dB at  7 MHz
//   S2 (100 pfu):   9 × 2 × (7/14) = 9 dB at 14 MHz
//                                    18 dB at  7 MHz
//   S3 (1000 pfu):  9 × 3 × (7/14) = 13.5 dB at 14 MHz
//                                    27 dB at  7 MHz (cap @ 30)
// Within NOAA's published S1-S3 expected-absorption ranges across the
// HF spectrum on polar paths.
//
// 2026-07-10 decade fix: the 2026-05-13 retune shipped with the driver
// divided by 10 (the S1 alert threshold) instead of the 1 pfu quiet
// background, which returned 0 dB at the actual S1 level and shifted
// every anchor above one NOAA S-class high (its "4.5 dB at S1" was in
// fact the value at S2). Referencing 1 pfu restores the documented
// anchors; the term remains continuous at its activation point, and
// sub-S1 proton enhancements now contribute a small (< 4.5 dB @ 14 MHz)
// polar-hop absorption consistent with riometer observations.
export function lPcaDb(fMHz, protonFluxP10, cgmLatAbsValue) {
  if (protonFluxP10 == null || protonFluxP10 <= PCA_FLUX_REF_PFU) return 0;
  if (cgmLatAbsValue == null || cgmLatAbsValue < PCA_CGM_THRESHOLD) return 0;
  if (!isFinite(fMHz) || fMHz <= 0) return 0;
  var driver = 9 * Math.log10(protonFluxP10 / PCA_FLUX_REF_PFU);
  if (driver <= 0) return 0;
  var L = driver * (7 / fMHz);
...
//, separate from the >=10 MeV main term's 9 dB/decade, because p1
...
// above its quiet-time floor (~1 pfu) but the p10-driven main term is
// still small, this bump keeps polar paths reflecting the imminent
// arrival of the harder population. Capped at 5 dB/hop; the caller
// takes max(lPcaDb, lPcaOnsetDb) per hop so the main term takes over
// smoothly once it exceeds this soft channel. Returns 0 if p1 is quiet
// or if the path doesn't traverse the polar cap.
export function lPcaOnsetDb(fMHz, protonFluxP1, protonFluxP10, cgmLatAbsValue) {
  ...
  // channel crossed the S1 alert threshold (10 pfu), which produced up to a
...
// Band-independent hop-geometry memo for pathIonoLosses. The per-point
// (lat, lon, cosZ, cgm) values and the floor/ceil hop layouts depend
// only on the path endpoints, distance, hF, and time - not on the band
// - yet the budget evaluates pathIonoLosses once per band (12x per path
// per refresh; 10x per cell in harness sweeps). Caching the geometry
// makes the per-band call pure arithmetic over precomputed points.
// Wholesale clear on overflow: 32k entries comfortably holds a full
// browser refresh (~1100 paths) and a full harness window (~25k cells),
// and the rebuild cost after a clear is one geometry pass.
var _geomCache = new Map();
var _GEOM_CACHE_MAX = 32768;

function _pathGeom(midLat, midLon, srcLat, srcLon, dstLat, dstLon,
                   dKm, date, hF) {
  var key = midLat + "," + midLon + "," + srcLat + "," + srcLon + "," +
            dstLat + "," + dstLon + "," + dKm + "," + hF + "," +
            date.getTime();
  var hit = _geomCache.get(key);
  if (hit) return hit;
  ...
  // Reflection-point list for an integer hop count, with per-point
  // solar zenith and CGM latitude precomputed. Falls back to the
  // midpoint when geometry is missing.
  function _pointsAt(n) {
    ...
    if (n <= 1 || srcLat == null || dstLat == null) {
      ...
      for (var k = 1; k <= n; k++) {
        ...
        if (pt != null) raw.push(pt);
      }
    }
    var out = [];
    for (var i = 0; i < raw.length; i++) {
      out.push({
        cosZ: solarCosZenith(raw[i][0], raw[i][1], date),
        cgm:  cgmLatAbs(raw[i][0], raw[i][1])
      });
    }
    return out;
  }
  ...
  var geom = {
    alpha: alpha,
    ptsFloor: _pointsAt(nFloor),
    ptsCeil:  null
  };
  if (alpha > 0) geom.ptsCeil = _pointsAt(nCeil);

  if (_geomCache.size >= _GEOM_CACHE_MAX) _geomCache.clear();
...
export function pathIonoLosses(fMHz, kp, hpGw, protonFluxP10, xrayClass,
  ...
  // precomputed points. Pure summation; caps applied later.
  function _accumAt(pts) {
    ...
    for (var i = 0; i < pts.length; i++) {
      ...
      // on >=10 MeV flux) ramps from the 1 pfu quiet background; the
      // onset term (lPcaOnsetDb, keyed on >=1 MeV flux) provides early
      // warning before the harder channel rises. Summing them
      // double-counted near threshold; max gives a smooth handoff.
      L.lPca   += Math.max(
  ...
  // Blend losses for the two integer hop counts bracketing the
  // continuous nC, weighted by the fractional part alpha. When nC is
  // exactly integer (alpha = 0), the ceil branch is identical to the
  // floor branch and no blend happens. Geometry (hop layouts, per-point
  // solar zenith and CGM latitude) is band-independent and memoized.
  var geom  = _pathGeom(midLat, midLon, srcLat, srcLon, dstLat, dstLon,
                        dKm, date, hF);
  var alpha = geom.alpha;

  var Lf = _accumAt(geom.ptsFloor);
  var Lc = (alpha > 0) ? _accumAt(geom.ptsCeil) : Lf;
...
// Parsed once at module load; re-deriving Object.keys().map(parseFloat)
// on every call allocated two arrays per noiseDbm invocation on the
// budget hot path (4 margin functions x ~6000 calls per refresh).
var _NOISE_KEYS = Object.keys(NOISE_FLOOR_DBM).map(parseFloat);

function _baseNoiseDbm(fMHz) {
  if (typeof fMHz !== "number" || isNaN(fMHz)) return null;
  var best = _NOISE_KEYS[0], bestD = Math.abs(fMHz - best);
  for (var i = 1; i < _NOISE_KEYS.length; i++) {
    var d = Math.abs(fMHz - _NOISE_KEYS[i]);
    if (d < bestD) { best = _NOISE_KEYS[i]; bestD = d; }
  }
...
export function noiseDbm(fMHz, opts) {
  ...
  // Atmospheric channel with diurnal swing (P.372 Fig 15). NOISE_FLOOR_DBM
  // is the *daytime* floor (see the table's header note), so the swing is
  // strictly one-sided in practice: the night side of diurnalNoiseShape
  // lifts the channel above the floor, and its day side is clamped away
  // because the galactic background is what it is and the table already
  // sits on it. Written as an explicit max(0, ·) rather than the previous
  // add-then-clamp, which computed a negative daytime offset and then
  // discarded it - arithmetically identical, but it read as if daytime
  // noise reduction were modelled when it never was.
  var atmo = base;
  if (opts.cosZenithNow != null) {
    atmo += Math.max(0, diurnalNoiseShape(fMHz, opts.cosZenithNow));
  }
