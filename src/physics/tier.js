...
// (see docs/HF-TIER-AGGREGATION.md).
//
// STATUS (2026-08-01): not in the live verdict path. deriveConditions
// settled on the worldwide-hub mean (Option A in that doc); this and
// TIER_COVERAGE_TARGET remain exported, unit-tested, and documented as
// the alternative aggregator the doc compares against. Kept
// deliberately - do not read a call site into the doc's description of
// it. PURE function of the margin
...
export function reliability(margin, sigma) {
  if (margin == null || !isFinite(margin)) return 0;
...
var _TIER_BOUNDARIES = [TIER_DB_POOR, TIER_DB_FAIR, TIER_DB_GOOD, TIER_DB_EXCELLENT];

export function tierStability(margin, sigma) {
  if (margin == null || !isFinite(margin)) return 0;
  var s = (sigma != null && sigma > 0) ? sigma : DEFAULT_SIGMA_DB;
  var minDist = Infinity;
  for (var i = 0; i < _TIER_BOUNDARIES.length; i++) {
    var d = Math.abs(margin - _TIER_BOUNDARIES[i]);
