...
// the noon-time floor; the diurnal swing in diurnalNoiseShape
// (src/physics/loss.js -- NOT lAbsDiurnalDb, which is the D-region
// absorption term and was named here by mistake) adds back
...
// at midlatitudes (30° to 60°, the gate in climatology.js) vs summer.
// Driven by O/N2 seasonal shift.
...
//   grayline bonus  = min(15, dayLoss(f)) * exp(-(cosZ/0.25)^2)   (credit)
//
// (The grayline form is the terminator-peaked Gaussian in
// _grayLineBonusPoint, src/physics/modes.js - 0.2 dB pre-asymmetry
// floor, 0.5x sunset multiplier, path-averaged over 5 samples. An
// earlier dayLoss * (1 - cos^0.7) "refund" shape paid the full credit
// for any cosZ <= 0 and over-credited every past-sunset path; the
// Gaussian decays into deep night as well as deep day.)
//
...
// Sauer-Wilkinson-style: active when GOES >=10 MeV proton flux is elevated
// above the ~1 pfu quiet-time background AND the path traverses the polar
// cap (|CGM| >= 60°). Magnitude scales log10(flux / 1 pfu) with f^-1.0.
// At the NOAA S-scale thresholds this yields 4.5 dB at S1 (10 pfu) /
// 14 MHz, 9 dB at S2 (100 pfu), 13.5 dB at S3 (1000 pfu) - 27 dB at
// S3 / 7 MHz, before the 30 dB per-hop cap.
//
// 2026-07-10 decade fix: the driver previously divided flux by 10 (the S1
// alert threshold), which returned 0 dB at the actual S1 level and shifted
// every documented anchor one NOAA S-class high (the "4.5 dB at S1" figure
// was in fact the value at S2, etc.). Referencing the quiet background
// (1 pfu) restores the documented anchors and keeps the term continuous
// at its activation point.
export const PCA_FLUX_REF_PFU       = 1;
