...
|---|---|---|
| `push.yml` | every push / PR | Fast unit + i18n + parse-check (5 min); rebuilds the whitepaper PDF and auto-commits it when the LaTeX source changed. Catches regressions before merge. |
| `daily.yml` | 05/11/17/23 UTC + 06:00 UTC | Tropo GFS grid refresh to R2 on the GFS cycle cadence; at 06:00 the WSPR baseline auto-commit, then all non-heavy suites (catches network-side drift: RBN, PSKReporter, WSPR-SNR, VOACAP cross-check, Hepburn tropo drift), plus the lychee link scan. |
| `monthly.yml` | 09:00 UTC on the 3rd | GIRO station audit (issue on drift), SWPC schema check (PR on drift), tropo Hepburn autotune (ledger auto-commit + guarded PR), and the full heavy suite (`tune-r7`, `voacap-fixtures`). |
| `annually.yml` | 04:00 UTC 20 October | IMO calendar refresh (guarded PR), citation review tickler, solar-cycle retune review (issue, never auto-commits), and the IGRF / ITU-R / cycle-phase watchdogs. |

Note on schedule liveness: GitHub disables cron workflows after 60
days without repo activity. The daily WSPR baseline commit keeps the
activity clock reset under normal operation; if wspr.live were down
for two months straight, re-enable the schedules by hand from the
Actions tab (the old keepalive.yml watchdog was removed 2026-08-03).
...
- **WSPR spot baselines** (`src/data/spot-baselines.mjs`). Captures the
  ...
  `.github/workflows/daily.yml` (cron, daily 06:00 UTC)
