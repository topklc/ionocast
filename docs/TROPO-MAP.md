# Tropospheric ducting heatmap

**This subsystem is production-wired.**  The UI panel is mounted by
`src/ui/builders/tropo-map.js` (registered via a `{ type: "tropo-map" }`
block in `src/ui/sections.js`), which imports `src/tropo/runtime.mjs`;
the runtime fetches the packed grid from the production R2 bucket at
`data.ionocast.org/tropo/grid.bin`.  Ingest runs on a GitHub Actions
cron (`.github/workflows/daily.yml` (tropo job), 4× daily) that pulls GFS,
computes the index, packs the binary, and uploads to R2.  Deleting
`src/tropo/` would break the deployed tropo panel.
...
it on load and bicubic-resamples the cells onto a 1440 × 720 Web
...
- **Heatmap fill**; per-cell tropo index, bicubic-interpolated, mapped
  to a 15-band Hepburn-style discrete color ramp:
    midnight blue → navy → blue → cyan → green → yellow →
    gold → orange → deep red.
  Band fills render with `fill-antialias: true` for clean edges while
  keeping the meteorological-forecast contour-fill look.
- **Black isolines**; marching-squares contours computed on the same
  grid: heavy lines at the two P.453 class boundaries (CUT_STANDARD =
  20, CUT_DUCTING = 90 M-units) plus two derived light guide levels
  (the super-refractive and ducting midpoints), rendered as thin
  black lines on top of the bands.
...
- `tropo_index`; composite radio-relevant ducting metric.  Sum of
  ...
  - **Moisture gate** (added 2026-07): the two temperature-inversion
    terms (layer + surface) are scaled by
    `clamp(e_2m / 12 hPa, 0, 1)` (reference vapour pressure
    configurable via `TROPO_MOIST_E_REF`) to suppress dry polar
    radiative-inversion false positives.  Validated: Antarctic share
    of strong-ducting cells dropped 60.3 % → 1.0 % with the moist
    ducting field untouched.
  ...
  **Convection penalty**: a super-adiabatic lapse (steeper than
  ...
  P.453 classifications places the ITU-R cuts at
  `standard < 20 ≤ super-refractive < 90 ≤ ducting`
  (CUT_STANDARD = 20, CUT_DUCTING = 90) with ducting recall 63 % /
  precision 71 % (post-moisture-gate, GFS 2026-07-01 06z).
- `surface_duct`; boolean.  True iff the lowest trapping layer's
  base sits ≤ 50 m above the model orography (`groundHm + 50`).
...
- **GFS resolution ceiling.**  Calibration against radiosondes
  shows our index has 71 % ducting precision and 63 %
  recall (post-moisture-gate, GFS 2026-07-01 06z); most missed
  cases are surface and shallow ducts in
...
- **Operational validation depth.**  Hepburn has run for ~20 years
  ...
  ~13 amateur-relevant sondes per cycle, ducting recall 63 % /
  precision 71 %, post-moisture-gate, GFS 2026-07-01 06z).
  A continuous nightly run accumulating over weeks
...
## How it is wired into production

`src/tropo/` started life as a self-contained sandbox but is now a
production subsystem:

- `src/ui/builders/tropo-map.js` imports `src/tropo/runtime.mjs` and
  mounts the MapLibre panel; `src/ui/sections.js` registers it via a
  `{ type: "tropo-map" }` block.
- The runtime fetches the packed binary grid from the production R2
  bucket at `data.ionocast.org/tropo/grid.bin` (JSON and local-file
  fallbacks for development).
- Ingest runs on the `.github/workflows/daily.yml` (tropo job) cron
  (4× daily, ~5 h after each GFS cycle): ingest → `pack-binary.mjs`
  → upload to R2.
- The output grid and GRIB cache stay gitignored; only the R2 object
  is deployed.

Changes to the heatmap, the index physics, or the color ramp are
therefore changes to a deployed surface.
...
- **Native binary grid format.**  DONE: `pack-binary.mjs` implements
  this (header struct + packed `Float32Array`, parsed in
  `runtime.mjs` via `DataView`) and is the production path; the
  tropo-ingest workflow packs and uploads `tropo/grid.bin` to R2.
...
## Production migration checklist (DONE as of 2026-07)

Kept as a historical record; every item below is completed.

1. ~~Move `ingest.mjs` to a scheduled runner, running every cycle
   (4× daily).  Output to a Cloudflare R2 bucket in a compact binary
   format (Float32Array packed) instead of JSON.~~  Done via the
   `.github/workflows/daily.yml` (tropo job) GitHub Actions cron (rather
   than a Cloudflare Worker) + `pack-binary.mjs`.
2. ~~Replace the renderer's `fetch("data/grid.json")` with a
   fetch of the R2-hosted binary.~~  Done: `runtime.mjs` fetches
   `https://data.ionocast.org/tropo/grid.bin` with fallbacks.
3. ~~Register a `tropo-map` builder under `src/ui/builders/` and add
   a `{ type: "tropo-map" }` block in `src/ui/sections.js`.~~  Done.
4. ~~Switch MapLibre from local-vendored to either kept-vendored
   (preferred, control + offline) or Protomaps `.pmtiles` from R2
   if a base map ever becomes desirable.~~  Done: MapLibre remains
   vendored.

The index physics and the rendering pipeline shipped as-is; the
deployment plumbing above completed the migration.
