...
### ~~Atmospheric noise floor clamp vs paper formula~~ - RESOLVED 2026-07-03

Resolved in the 2026-07-03 whitepaper refresh: the paper's noise
formula now carries the explicit `max(0, ΔN_diurnal)` clamp, the rural
worked example reads −121 dBm, and fig:noise was re-anchored to the
current constants (−113/−98). Kept below for the record.

### Atmospheric noise floor clamp vs paper formula (original notes)
...
### Deferred from the 2026-07-03 full-system audit

Everything actionable from `docs/AUDIT-2026-07-03.md` shipped same-day
except these, each deferred for a stated reason:

- **Thickness-weighted super-refractive sum** (`ingest.mjs`): the SR
  term still scores per *layer*, not per metre, so the tropo index is
  coupled to `LEVELS_HPA` - a future level-set change silently
  re-scales the marine carpet. The 2026-07-03 min-thickness guard +
  sub-150 m taper removed the noise blow-up, but full per-metre
  weighting changes the index scale and needs a paired re-sweep of the
  renderer cuts (currently 20/90) against the sonde ground truth.
- **Fuse kriging observation-correlation handling**: N clustered
  stations each add independent 1/σ² weight, over-weighting dense
  regions relative to the prior. Real kriging with a correlation
  model is a v2-scale change; harmless at current station densities.
- **Stability % on spot-override rows**: the displayed percentage
  describes the pre-override physics margin while the tier shows the
  overridden "good". Suppressing or relabeling it means changing
  behavior the whitepaper now documents - do both sides together.
- **`tr` locale**: 189 missing keys (i18n suite report), plus one new
  key from the envelope label ("multi-lead envelope from …").
  Translation work, not code.

### Deferred from the 2026-08-01 physics-engine audit

Findings 1-3 and P1-P2/D1-D5 of `docs/AUDIT-2026-08-01.md` shipped
same-day. These are the items that were deliberately *not* changed,
each because it needs a harness pass this environment could not run
(no Node available) or a paper rebuild (no LaTeX available):

- **Per-hop fuse sampling in `pathMinMuf`** (enhancement, not the bug).
  The 2026-08-01 fix restores the documented invariant by feeding
  `pathMinMuf` a pure-climatology midpoint denominator, so its per-hop
  profile is once again a pure shape correction. The strictly better
  version samples the *fuse grid* at each reflection point, which
  brings observations along the whole path (not just the midpoint) into
  the per-hop minimum. Needs its own harness pass and a resolution
  decision: the fuse grid's 10° nodes (~1100 km) smooth the terminator
  gradient that this term exists to resolve, so a naive swap could
  weaken the night-hop pessimism it was calibrated for. Wants either a
  finer foF2 grid or a hybrid (climatology shape × fuse-observed
  per-hop corrections where an observation is actually in range).
- **Takeoff-angle / loss-sampler hop-geometry reconciliation** (audit
  P3). `takeoffAngleDeg` and `mufObliqueFactor` use
  `d_hop = min(d, ceiling)`; `pathIonoLosses` uses the bracketed
  floor/ceil(nC) layout. They agree at single-hop and ceiling-saturated
  paths, diverge ~1.3° of takeoff angle at 6000 km. Moving takeoff
  angle onto the bracketed-blend form shifts `gAnt` on every multi-hop
  path, so it is a calibration change, not a cleanup. Documented in
  `geometry.js` in the meantime.
- **Gray-line magnitude re-check** (audit P5). Not a bug: the paper
  (`sec:grayline`) explicitly re-bases the term as a credit relative to
  a fully-dark path (terminator tilt / ducting / antipodal focusing),
  with `L_day(f)` as a magnitude anchor rather than a derivation, and
  the magnitude is WSPR-calibrated like every other empirical term.
  Worth one look anyway because it is the largest additive term in the
  budget and sits where tier boundaries are most sensitive: on 160 m it
  pays +15.0 dB at `cosZ = 0` and +14.4 dB at `cosZ = -0.05` against a
  0 dB absorption charge, i.e. it asserts a terminator path is two full
  tier boundaries better than the same path at midnight. Note the
  2026-08-01 `lAbsDiurnalDb` twilight ramp now charges up to 4.45 dB
  (160 m) in the `cosZ ∈ [0, 0.05]` sliver where the credit peaks, so
  the net terminator position has moved slightly and any re-fit should
  postdate that change.
- **Paper corrections pending a rebuild.** No LaTeX toolchain was
  available, and editing the `.tex` without rebuilding
  `ionocast-methodology.pdf` would desync the committed PDF. Two
  passages to fix on the next paper pass: (a) `sec:labsd` says "At
  night (cos χ < 0.05) the term returns zero" - it is now a linear
  ramp over `[0, 0.05]`, matching the flare term; (b) `sec:mufoblique`
  claims the shared `_hopElevationRad` helper means antenna-gain and
  oblique-MUF sampling "can never disagree about hop geometry" - true
  between those two, but not against `pathIonoLosses` (see P3 above).
...
- `hp30.js`, `silso.js`, `tropo.js`, `refractivity.js`
