...
\SI{36.2}{\dB}$, a value bracketed by the prior table's
...
frequencies, $L_{\text{day}}(f)$ from Eq.~\ref{eq:labsd}, replace
the prior Table~2 anchors:
\begin{equation*}
\begin{array}{lrlr}
\textbf{Band} & {f}~(\si{\MHz}) &
\textbf{Band} & {f}~(\si{\MHz})\\
...
\textbf{Band} & f & & B_{\max} \\
\midrule
160\,\text{m} & 1.85~\si{\MHz} & \to & 25~\si{\dB}~(\text{capped}) &
...
($+30$ to $+50\,\si{dBm}$ for the curated amateur-beacon table;
...
radial-basket rings (2500 / 4000 / 6000 / 9000 / 12000 / 16000\,km).
...
accumulates over the radial basket so a strong short-path
