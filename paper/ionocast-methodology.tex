...
ten reference propagation paths (five short-path destinations
plus their long-path counterparts); the SNR budget runs per band,
per path, at the current time.  A forward-projection MUF pipeline (solar-zenith
...
each of ten reference paths --- great circles from the operator's
QTH to five canonical short-path destinations (New York, S\~ao
Paulo, Johannesburg, Tokyo, Sydney) and their five long-path
counterparts, midpoints recomputed per-QTH on each reload ---
ionocast selects the nearest ionosonde to that path's midpoint.
...
with the longitude wrapped back into $[-180\degree, +180\degree]$ before
the lookup so the kc2g grid index is well-defined.
...
ITU-R P.842 reliability buckets in $\sigma$-units rather than by
a fixed dB-width tier scheme, so the per-band absolute margin
bias has no constant width to fit against.
...
budget-shaping parameter; it is fixed for reproducibility.
...
\S\ref{sec:tier-confidence} (near-MUF, storm, forecast storm,
cross-terminator, storm-recovery TID) plus the Es-active
contribution defined in \S\ref{sec:es-prop-mode} capture the
within-hour spread that P.842's static day-to-day $\sigma$ does
...
its assigned value (per Table~\ref{tab:bandsigma}: $\sigma_g{=}9$
on 17\,m, $\sigma_g{=}10$ on 15\,m, $\sigma_g{=}12$ on 12\,m).
``$\Delta$ tier (full sweep)'' counts the tier flips across the
three sweep points.  17\,m and 15\,m both flip; 12\,m does not.
At 17\,m the Fair/Good boundary at the assigned $\sigma_g{=}9$
sits at $z \cdot \sigma_g = 0.2533 \cdot 9 = \SI{2.28}{\dB}$, so
$M = +\SI{2.5}{\dB}$ lands just inside Good ($z = 0.278$,
$R = 0.61$); at $\sigma{+}2{=}11$ the boundary moves to
\SI{2.79}{\dB} and the same $M$ falls into Fair ($z = 0.227$,
$R = 0.59$).  At 15\,m the boundary at $\sigma_g{=}10$ sits at
\SI{2.53}{\dB}, just above $M = +\SI{2.5}{\dB}$ ($z = 0.25$,
$R = 0.60$, Fair); at $\sigma{-}2{=}8$ the boundary tightens to
\SI{2.03}{\dB} and $M = +\SI{2.5}{\dB}$ lands above it
($z = 0.313$, $R = 0.62$, Good).  12\,m has a
$0.2533 \cdot 12 = \SI{3.04}{\dB}$ Fair/Good gap which
$M = +\SI{2.5}{\dB}$ sits below at all three sweep points, so it
reads Fair throughout.}
\label{tab:sigmasens}
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Band} & $\sigma_g - 2$ & assigned $\sigma_g$ & $\sigma_g + 2$ & $\Delta$ tier (full sweep) \\
\midrule
17\,m & $\sigma=\phantom{0}7$, $R=0.64$, Good  & $\sigma=\phantom{0}9$, $R=0.61$, Good & $\sigma=11$, $R=0.59$, Fair & 1 tier (Good $\to$ Fair) \\
15\,m & $\sigma=\phantom{0}8$, $R=0.62$, Good  & $\sigma=10$, $R=0.60$, Fair & $\sigma=12$, $R=0.58$, Fair & 1 tier (Good $\to$ Fair) \\
...
($0.2533 \cdot \sigma_g = \SI{2.28}{\dB}$ at $\sigma_g{=}9$) of
the three near-MUF bands, so it crosses a boundary at lower $M$
values than 15\,m or 12\,m and is the most sensitive to
$\sigma_g$ miscalibration in practice.  This makes 17\,m the prime candidate for re-anchoring
...
\caption{Tier probability distribution for a margin of
         $+3$\,dB with $\sigma=8$\,dB under the tier boundaries
         in Table~\ref{tab:tiers}. The Fair band sits at
         $z \in [-0.385, +0.253]$ ($-3.08$\,\si{\dB} to
         $+2.03$\,\si{\dB} at $\sigma=8$, asymmetric); the
         Excellent / Closed cutoffs are symmetric at
         $\pm 1.282\sigma$ ($\pm 10.25$\,\si{\dB}). Most likely tier is Good
         ...
         \in \{8, 9, 10, 12\}$\,dB depending on band.  Read the figure
...
destination, which is material information for the operator.  A
median-margin aggregation would dominate the verdict with the
failing long paths because most reference paths are long
transcontinental hops that are routinely over-MUF or
night-crossing, burying genuinely open short paths (e.g.\ a
944\,km EM79--NYC path reporting \SI{+27}{\dB} margin would have
its verdict overwritten by the median of eight failing long
paths).
