...
With the per-band $\sigma$ for 20\,m = \SI{9}{\dB}
(Table~\ref{tab:bandsigma}), the standardised margin is
$M / \sigma = 33.1 / 9 = 3.68$. Since
$M / \sigma \ge 1.2816$ (the ionocast $R \ge 90\%$ Excellent threshold,
Table~\ref{tab:tiers}), the verdict is ``Excellent'' and the predicted
reliability is $\Phi(3.68) \approx 0.99988$ (effectively
$\sim$99.99\,\%; the tail beyond $z = 3.68$ is $\sim 1.2 \times 10^{-4}$).
The wide head-room is typical of midday 20\,m on quiet days at
1000\,km.
...
\max(B_{\text{TEP}}, B_{\text{scatter}})$) fed into the
P.842 reliability formula (Equation~\ref{eq:reliability}); the
WSPR spot-override of \S\ref{sec:spot-override} can additionally
promote a band-group summary tier from below-Good up to Good
when the activity ratchet fires, but does not enter the
per-band row's tier or stability columns.  The harness retains
a binary ``$\geq 50$ spots/h'' WSPR~\cite{wsprlive} activity
check as a sanity floor only --- it does not feed the runtime
verdict --- since spot activity measures who is transmitting
rather than whether a QSO would complete on the configured
station; the per-path mode of the harness uses a sparser
$\geq 1$ spot/h floor over TX/RX bbox-restricted aggregates,
documented in \S\ref{sec:calibrate} (Table~\ref{tab:harnessruns}).
...
\paragraph{Boundary anchoring.}  The $0.90 / 0.60 / 0.35 / 0.10$
$R$-schedule is anchored to operator failure-rate intuition:
Excellent $\approx 1$ in 10 fail, Good $\approx 4$ in 10 fail,
Fair the coin-flip-down band, Poor $\approx 9$ in 10 fail,
Closed essentially always.  Excellent / Closed boundaries are
symmetric at $\pm 1.2816\,\sigma$, so Excellent at $\sigma_g =
\SI{9}{\dB}$ sits at $\sim \SI{11.5}{\dB}$ (reachable on a
strong DX path) and Good at $+0.2533\,\sigma \approx
