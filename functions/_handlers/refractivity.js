...
export function recentSoundingTime(now) {
  ...
  // Wyoming usually publishes the 12z sounding by ~13:30 UTC (and 00z by
  // ~01:30), so switching at 14 / 02 UTC keeps a safety margin. The old
  // 15 / 03 switch left users on 13-15 h-old data for an extra hour.
  let target = h >= 14 ? 12 : (h >= 2 ? 0 : -12);
  if (target < 0) {
...
// Build the University of Wyoming sounding request URL for a launch slot.
//
// Wyoming retired the legacy CGI endpoint - the old form
//   /cgi-bin/sounding?region=..&TYPE=TEXT:LIST&YEAR=..&MONTH=..&FROM=ddhh&TO=ddhh&STNM=..
// now returns HTTP 404 for every request. The current WSGI endpoint takes a
// single `datetime` ("YYYY-MM-DD HH:00:00", UTC) plus the station `id`; the
// `region` no longer participates in the query and `src=UNKNOWN` lets the
// server resolve the station's own network. The response body is still the
// same <PRE>-wrapped PRES/HGHT/TEMP/DWPT column table parseSounding() reads,
// so only the URL construction changed.
export function uwyoSoundingUrl(slot, code) {
  const { yyyy, mm, ddhh } = fmtUwyoDate(slot);
  const datetime = `${yyyy}-${mm}-${ddhh.slice(0, 2)} ${ddhh.slice(2)}:00:00`;
  return "https://weather.uwyo.edu/wsgi/sounding"
       + `?datetime=${encodeURIComponent(datetime)}`
       + `&id=${encodeURIComponent(code)}`
       + `&src=UNKNOWN&type=TEXT%3ALIST`;
}
