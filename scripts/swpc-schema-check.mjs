...
// that pass through the daily suites run because the fetch still succeeds.
//
// Exit code: 0 on any completed run (diffs included - a non-empty
// diff on stdout is the workflow's signal to open an issue, not a
// failure); 2 only on an unexpected crash.  Transient fetch failures
// (5xx, timeouts) are skipped without contaminating the diff.
//
// Flags:
//   --write   When schema drift is detected, overwrite the committed
//             snapshot with the current fingerprint.  Used by
//             the monthly workflow to seed an auto-PR whose diff the
...
const ENDPOINTS = [
  ...
  // Kyoto Dst as republished by SWPC - consumed via the swpc-dst proxy
  // (functions/_proxies.js) and fetchDst(); was the one consumed
  // endpoint missing from this list (AUDIT-SCRIPTS-2026-08-03).
  { name: "kyoto-dst",    url: "https://services.swpc.noaa.gov/products/kyoto-dst.json",                       type: "json" },
];
