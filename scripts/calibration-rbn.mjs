...
// `--days` without =N would parse "true" -> NaN and silently load
// zero days; fall back to the default instead.
const DAYS = parseInt(args.days ?? "30", 10) || 30;
...
function path4(window, cache, kpAt) {
  console.log();
  console.log("=".repeat(72));
  console.log("PATH 4 - binary tier confusion (RBN as truth)");
  console.log("=".repeat(72));
  ...
  for (const skHr of activeSkHr) {
    ...
    for (const beacon of Object.keys(beaconInfo)) {
      ...
      for (const band of b.bands) {
        ...
        // and the reach gate lives in isDxOpen; DX-flag stratification
        // lives in path4b.  (A former `noReachGate` option here only
        // changed the section header - the with/without runs were
        // numerically identical - and was removed 2026-08-03.)
