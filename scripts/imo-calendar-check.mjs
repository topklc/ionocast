...
//             (same month, |delta| <= 3 days), rewrite the matching
...
// Exit non-zero on a crash so the annual workflow goes red instead of
// silently green: "fatal:" matches none of annually.yml (imo job)'s
// drift_signal grep patterns, so an exit-0 crash produced no PR, no
// issue, and a passing run - the annual reminder could die unnoticed.
