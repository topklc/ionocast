...
function rbnFetchDay(yyyymmdd) {
  ...
  try {
    // -f: fail on HTTP errors.  Without it curl exits 0 on a 404/500
    // HTML page, unzip then fails, and the `|| true` below used to
    // turn that into an empty CSV that was cached as a permanent
    // "zero spots" day (cache-poisoning; see AUDIT-SCRIPTS-2026-08-03).
    execFileSync("curl", ["-sfL", "--max-time", "120",
  ...
  let csv;
  try {
    // `grep || true` keeps a legitimate zero-match from failing the
    // pipeline, but a broken zip also lands here as empty output -
    // so an empty result is treated as a fetch failure and NOT cached.
  ...
  const rows = csv.split(/\n/).filter(Boolean).map(line => {
    ...
    // c[6] is the archive's own dx_pfx column (e.g. "K" for every US
    // call).  It used to be ignored in favor of re-deriving a prefix
    // from the spotted callsign, which sent W/N/A-prefixed US calls
    // to dropNoLoc (RBN_DXCC has K but no W/N/A) and skewed the
    // residual population.
    return { skimmer: c[0], freq: parseFloat(c[3]), band: c[4],
             tx: c[5], txPfx: c[6], mode: c[8], snrDb: parseInt(c[9], 10),
             dateStr: c[10], txMode: c[12] || c[8] };
  }).filter(r => r && isFinite(r.freq) && isFinite(r.snrDb));
  if (!rows.length) return null;   // do not poison the day cache
  writeFileSync(cacheFile, JSON.stringify(rows));
  // The zip is left in /tmp deliberately: rbn-beacon.mjs reuses it in
  // the same run instead of re-downloading ~100 MB.
  return rows;
}
...
async function _runRbn(opts) {
  ...
  for (const row of rows) {
    ...
    if (!rxLL) continue;
    // Prefer the archive's dx_pfx column; fall back to deriving from
    // the callsign only for pre-fix cached rows that lack txPfx.
    const txCall = (row.tx || "").toUpperCase();
    const derived = (txCall.match(/^[A-Z0-9]{1,3}/) || [null])[0];
    const txLL = dxccCentroid(row.txPfx || derived) || dxccCentroid(derived);
