...
export function runStormSplitSuite() {
  ...
  for (const s of samples) {
    ...
    const e = pOpen - a;
    // Unknown Kp (outside the history's ±4 h window) is neither storm
    // nor quiet - drop it rather than diluting the quiet bucket.
    if (s.kp == null) continue;
    const bucket = s.kp >= 5 ? "storm" : "quiet";
