#!/usr/bin/env node
// scripts/tests/physics-unit.mjs
//
// Rigorous direct tests for src/physics/physics.js, separate from the
// calibration harness tests in harness-unit.mjs (which only cover
...
//
// Run:  node scripts/tests.mjs --suite=physics-unit

// Only names with actual assertions below are imported - a former
// wider list (grayLineBonusDb, scatterBonusDb, snrMarginVhf*, the
// station-fusion interpolators, foF2Climatology, …) implied coverage
// this suite doesn't have; trimmed 2026-08-03 so the import list is an
// honest table of contents.
import {
  // distance + hop
  refDistanceHfKm, hopsForDistance, freeSpaceLossDb,
  // loss terms
  lMufDb, lAbsDb, lLowBandExtraDb, lAbsDiurnalDb,
  lHopGroundReflectionDb, lEsScreenDb,
  lAuroralDb, lPcaDb, lPcaOnsetDb, lFlareDb,
  // geometry
  cgmLatAbs, dipLatitude,
  // antennas
  antennaGainAtElevation,
  // bonuses
  tepBonusDb, irregularityRecoveryDb,
  // SNR margin
  snrMarginHf, snrMarginHfEs,
  // solar / climatology
  solarCosZenith, nvisSecantFactor,
  mufConsensus,
  // fuse v1 + v2
...
// ════════════════════════════════════════════════════════════════════
{
  ...
  // `lLowBandExtraDb` was rewritten (2026-04-30, alongside
  // lAbsDiurnalDb) from a stepping band-tier table to smooth log-space
  // interpolation through band anchors; the bound below now guards
  // that smoothness (any future re-introduction of a >4 dB step
  // between nearby frequencies fails here).
  {
    const { maxDelta, atF } = maxJump(lLowBandExtraDb, 0.5, 30, 0.1);
    check("lLowBandExtraDb: bounded step ≤ 4 dB (smooth interpolation)",
...
// ════════════════════════════════════════════════════════════════════
{
  ...
  // Above the Es MUF: no hard null since 2026-07-03 - the budget now
  // mirrors the F2 convention of smooth over-MUF handling, letting
  // lMufDb's 10 + 36·sqrt(r−1) branch bury the margin instead of
  // cliffing to closed at the MUF edge. Well over-MUF (f = 2.5×esMuf
  // here) the margin must be deeply negative and the over-MUF loss
  // dominant.
  const aboveMuf = snrMarginHfEs(50, 4, opts);  // esMuf=20, f=50
  check("snrMarginHfEs: well above Es MUF → deeply negative margin",
        aboveMuf && aboveMuf.margin < -15 && aboveMuf.lMuf > 30,
        `got=${JSON.stringify(aboveMuf)}`);
}
...
// ════════════════════════════════════════════════════════════════════
// 12. snrMarginHf: per-band continuity.
//
// Sweep across the 10 HF bands at the same path/conditions and assert
// no |Δmargin| > 18 dB between adjacent bands. Bounds every loss term
// snrMarginHf itself applies. NOTE: the TEP bonus is NOT one of them -
// it is folded in downstream (src/derive/conditions.js via
// irregularityRecoveryDb), so a TEP regression is caught by the direct
// tepBonusDb sweep in section 4, not here.
// ════════════════════════════════════════════════════════════════════
{
  const opts = {
  ...
  check("snrMarginHf: band-to-band Δ < 18 dB (continuity bound)",
        maxJump < 18, `max ${maxJump.toFixed(2)} dB at ${atBand}`);
}
...
// ════════════════════════════════════════════════════════════════════
{
  ...
  check("antennaGain dipole 10m peak in 20-60° elevation",
