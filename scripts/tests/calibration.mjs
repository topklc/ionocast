...
//
// Reliability curve (decile bins), P.842 tier confusion, and a rolling
// in-sample vs held-out split of the cache window. Uses per-path truth
// when the cache has wsprByPath, otherwise global.

import {
  BANDS, DEFAULT_CONFIG, makeCellData, makeStationsAt,
  normCdf, replayMarginFromCell,
  BINARY_OPEN_FLOOR_GLOBAL, BINARY_OPEN_FLOOR_PERPATH,
} from "../harness.mjs";
...
export function runCalibrationSuite() {
  ...
  const openFloor = truthMode === "per-path"
    ? BINARY_OPEN_FLOOR_PERPATH : BINARY_OPEN_FLOOR_GLOBAL;
  // Rolling split: the harness cache only ever holds the trailing
  // <= 30 days, so a fixed calendar cutoff goes stale within a month
  // (the original 2026-04-26 constant left "in-sample" permanently
  // n=0; see AUDIT-SCRIPTS-2026-08-03).  Instead, hold out the newest
  // 7 UTC days present in the cache and treat the rest of the window
  // as in-sample.
  const HELD_OUT_DAYS = 7;
  let maxTs = -Infinity;
  for (const w of cache.wsprRows) {
    const ts = Date.parse(`${String(w.day)}T${String(Number(w.hour_utc)).padStart(2, "0")}:00:00Z`);
    if (isFinite(ts) && ts > maxTs) maxTs = ts;
  }
  const HELD_OUT_START = isFinite(maxTs)
    ? Date.UTC(new Date(maxTs).getUTCFullYear(), new Date(maxTs).getUTCMonth(),
               new Date(maxTs).getUTCDate() - (HELD_OUT_DAYS - 1))
    : Date.now();
  const WINDOWS = {
