...
function* walk(dir) {
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    // Vendored third-party bundles are not our source: the minified
    // MapLibre bundle alone minted 142 garbage keys (`it("…")`,
    // `at("…")` matching an unanchored `t(` regex) - ~16 % of the
    // template before this exclusion.
...
function collectSourceKeys() {
  ...
  for (const fp of walk(SRC)) {
    ...
    // Lookbehind anchors so `it(`, `at(`, `x.t(` etc. never match -
    // only a bare function call spelled exactly t( / abbr( / … counts.
    extractAll(/(?<![\w$.])t\(\s*"((?:\\.|[^"\\])*)"/g, text, keys);
    extractAll(/(?<![\w$.])t\(\s*'((?:\\.|[^'\\])*)'/g, text, keys);
    extractAll(/(?<![\w$.])abbr\(\s*"((?:\\.|[^"\\])*)"/g, text, keys);
    extractAll(/(?<![\w$.])abbr\(\s*'((?:\\.|[^'\\])*)'/g, text, keys);
    extractAll(/(?<![\w$.])pendingNote\(\s*"((?:\\.|[^"\\])*)"/g, text, keys);
    extractAll(/(?<![\w$.])pendingNote\(\s*'((?:\\.|[^'\\])*)'/g, text, keys);
  }
  // Label tables in settings.js whose VALUES flow into t() at runtime
  // (settings-ui renders t(ANT_TYPE_LABEL[k]) etc.).  Without this the
  // strings are invisible to extraction and can never be translated.
  const settingsSrc = fs.readFileSync(path.join(SRC, "settings.js"), "utf-8");
  const tableRe = /_LABEL\s*=\s*{([^}]*)}/g;
  let tm;
  while ((tm = tableRe.exec(settingsSrc)) !== null) {
    extractAll(/:\s*"((?:\\.|[^"\\])*)"/g, tm[1], keys);
    extractAll(/:\s*'((?:\\.|[^'\\])*)'/g, tm[1], keys);
  }
