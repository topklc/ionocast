  ...
  for (const p of VOACAP_BASKET) {
    ...
    if (muf != null) {
      ...
      const m = snrMarginHf(p.fMHz, muf, {
        ...
        // Path midpoint, matching every other suite and production -
        // the TX endpoint understated auroral loss on high-latitude
        // fixtures (FN30→JA's midpoint is near-Arctic; NYC's is not).
