...
async function pskFetchBand(intMHz) {
  // Day-keyed cache: the un-dated `tests-psk-<MHz>.xml` name replayed
  // the very first 1-hour PSKReporter snapshot forever.
  const day = new Date().toISOString().slice(0, 10);
  const cacheFile = resolve(CACHE_DIR, `tests-psk-${day}-${intMHz}.xml`);
