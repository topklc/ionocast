...
// not exercised by physics-unit.mjs (which only tests pure physics
...
//
// Run with: node scripts/tests.mjs --suite=derive-unit

// localStorage stub for the settings/i18n/qth modules. Note the timing
// subtlety: static imports hoist, so this assignment actually runs
// AFTER those modules evaluate - which is fine because none of them
// touch localStorage at module load (all reads are lazy, inside
// try/catch). The stub covers the lazy reads during test execution.
const _ls = new Map();
...
// ── 2. bzForwardKpBump ────────────────────────────────────────────────
{
  ...
  // Too few samples (need >= 3 in the window; threshold was lowered
  // from 10 for 5-min-cadence data, see src/derive/storm.js) -> 0.
  const tooFew = [bzSample(NOW, 5, -20), bzSample(NOW, 6, -20)];
  eq("bzBump: only 2 samples (need >=3) -> 0",
