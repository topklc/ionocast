...
export async function runWsprSnrSuite(opts = {}) {
  ...
  return {
    // `skipped` is reserved for skip-REASON strings across the suite
    // registry (tests.mjs prints `w.skipped || w.error` as text); the
    // per-spot skip count lives under nSkipped so a nonzero count
    // doesn't shadow the real summary.
    n: residuals.length, nSkipped, window,
