...
function rbnBeaconFetchDay(yyyymmdd) {
  ...
  if (!existsSync(tmp)) {
    try {
      // -f: fail on HTTP errors so an error page never reaches unzip
      // (which would cache an empty day; see AUDIT-SCRIPTS-2026-08-03).
  ...
  // Empty output means the zip was broken or the filter chain failed -
  // treat as a fetch failure rather than caching a permanent zero-spot
  // day.  (Genuine zero beacon spots over a whole UTC day does not
  // happen on a working archive.)
...
export function runRbnBeaconSuite() {
  ...
  for (const row of spots) {
    ...
    // RBN skimmers report as CALL or CALL-N (multiple instances at one
    // site); the grid table is keyed by bare call, so strip the suffix
    // (same normalization calibration-rbn.mjs documents).
    const rxGrid = BEACON_SKIMMER_GRID[row.skimmer]
                ?? BEACON_SKIMMER_GRID[row.skimmer.replace(/-\d+$/, "")];
