...
async function fetchStatic() {
  ...
  const fetchers = {
    ...
    // fetchDst hits the registered /api/swpc-dst proxy and returns
    // { dst, history, ... }.  The previous raw jproxy("/api/kyoto")
    // targeted a proxy name that doesn't exist (404 → silent null),
    // so every verdict ran with dst = null.
    kyoto:          () => fetchDst().catch(() => null),
  };
  const out = {};
