...
//   {
//     epoch:   "YYYY M D H M S",            // first successful center's epoch
//                                            // (centers are same-day products,
//                                            // so epochs agree to within hours)
...
const CENTERS = [
  ...
  },
  {
    ...
    // IGS short-name rapid product is JPRG (JPLG is the final) - JPLR
    // matches no IGS product and could never fetch.
    urlFor:      (d) => `https://cddis.nasa.gov/archive/gnss/products/ionex/${d.getUTCFullYear()}/${doy(d)}/JPRG${doy(d)}0.${String(d.getUTCFullYear()).slice(-2)}I.Z`,
    decompress:  "lzw",
    auth:        "earthdata",
  },
  {
  ...
  },
  {
    ...
    // IGS short-name rapid product is UPRG (UPCG is the final) - UPCR
    // matches no IGS product and could never fetch.
    urlFor:      (d) => `https://cddis.nasa.gov/archive/gnss/products/ionex/${d.getUTCFullYear()}/${doy(d)}/UPRG${doy(d)}0.${String(d.getUTCFullYear()).slice(-2)}I.Z`,
    decompress:  "lzw",
    auth:        "earthdata",
  },
];
...
async function fetchCenter(center, date) {
  ...
  if (center.decompress === "gzip") {
    ...
    // spawnSync never throws on a missing binary - it reports ENOENT
    // via res.error - so the availability check must read that field
    // (the old try/catch around it was unreachable).
    const { spawnSync } = await import("node:child_process");
    const res = spawnSync("uncompress", ["-c"], { input: buf });
    if (res.error) {
      return { name: center.name, error: "no uncompress binary (" + res.error.code + "); install with `apt install ncompress`", url };
    }
    if (res.status === 0) {
      text = res.stdout.toString("utf-8");
    } else {
      const errTxt = res.stderr ? res.stderr.toString("utf-8").trim() : "";
      return { name: center.name, error: "uncompress: " + (errTxt || "exit " + res.status), url };
    }
  } else {
...
async function main() {
  ...
  const results = await Promise.all(CENTERS.map(async (c) => {
    for (const d of tries) {
      ...
      // Missing-token skip is structural - no point walking older
      // dates.  HTTP/parse errors DO fall through to older dates
      // (rapid products post with up to ~2 days of latency).
  ...
  // Exit non-zero when nothing was fetched: a blind cron piping this
  // to an upload would otherwise clobber the last good stored GIM with
  // an empty ensemble (same last-good-preservation policy as
  // the daily workflow's tropo job).
  if (!liveCenters.length) {
    process.stderr.write("[gim-ensemble] every center failed:\n");
