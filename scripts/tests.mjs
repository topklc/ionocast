...
import { runUnitTests as runTropoCodec }  from "./tests/tropo-codec.mjs";
...
import { runHepburnSuite }         from "./tests/hepburn.mjs";
...
// Bare positional args are NOT suite selectors and used to be silently
// ignored - `node scripts/tests.mjs tune-r7` ran ALL non-heavy suites
// (which is how the annual retune workflow ran for months without ever
// executing tune-r7).  Fail loudly instead.
const positional = argv.filter(a => !a.startsWith("--"));
if (positional.length) {
  console.error(`unexpected positional arg(s): ${positional.join(", ")}`);
  console.error(`did you mean --suite=${positional.join(",")} ?`);
  process.exit(1);
}
...
const SUITES = {
  ...
  hepburn:          runHepburnSuite,
  // Heavy suites (gated behind --heavy).
  "tune-r7":        () => runTuneR7({ frozen: ["lIonoHfDb", "defocusDbPerExtraHop"] }),
  "voacap-fixtures": () => runVoacapFixtures(),
};
const UNIT_SUITES = new Set(["physics-unit", "harness-unit", "derive-unit", "tropo-codec", "i18n"]);
const NETWORK_SUITES = new Set(["wspr-snr", "rbn", "rbn-fuse", "rbn-beacon", "psk", "hepburn"]);
const HEAVY_SUITES = new Set(["tune-r7", "voacap-fixtures"]);
const ALL_NAMES = Object.keys(SUITES);

const SUITE_DESCRIPTIONS = {
  "physics-unit":    "~680 assertions: physics functions, frequency continuity, tier classification.",
  ...
  "tropo-codec":     "Round-trip tests for the shared TROPO binary grid codec (encode → decode, envelope flag, unaligned buffers, corruption rejection).",
  ...
  hepburn:           "Production tropo grid rank-correlated against Hepburn's ducting maps (eam/eur/wam). Skips without python3/network; throws below rho floor 0.35.",
...
if (EMIT_JSON) {
  ...
  for (const u of ["physics-unit", "harness-unit", "derive-unit", "tropo-codec"]) {
    if (results[u]) {
      const r = results[u];
