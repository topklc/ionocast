...
//
// Imports from src/physics/physics.js resolve as ES modules via the
// .mjs extension of this file (no package.json exists or is needed;
// Node's ESM syntax detection handles the imported .js files).
...
import {
  // Pure-physics imports, no tunable constants inside.  Only names the
  // harness actually calls are imported (a former block also pulled in
  // lAbsDb / lEsScreenDb / lPcaDb / lPcaOnsetDb / lFlareDb /
  // mufConsensus / pathMinMuf / nightFloor / grayLineBonusDb and three
  // reference constants that nothing read - implying margin-path
  // coverage that didn't exist; trimmed 2026-08-03).
  lMufDb,
  lAbsDiurnalDb,
  lLowBandExtraDb,
  lAuroralDb,
...
// Six earlier coord errors were found and fixed (via what is now the
// `harness.mjs verify` subcommand); the prior values were operator
...
async function fetchF107() {
  ...
  const data = await r.json();
  // Do NOT assume feed ordering: the SWPC json has been observed
  // newest-first (2026-08), and an oldest-first assumption here served
  // a ~6-week-stale `current` for months.  Sort by time_tag descending
  // explicitly, then `current` = newest Afternoon flux and `mean81` =
  // mean over the newest <= 81 Afternoon observations.
  const afternoon = data
    .filter(d => d.reporting_schedule === "Afternoon" && d.flux != null && d.time_tag)
    .sort((a, b) => Date.parse(b.time_tag) - Date.parse(a.time_tag));
  const obs = afternoon.slice(0, 81).map(d => +d.flux);
    ...
    if (cached.schemaVersion !== CACHE_SCHEMA_VERSION) {
      ...
      if (cached.windowDays === days && ageH < 24) {
        ...
        if (mode === "per-path" && (!cached.wsprByPath || cached.bboxDeg !== bboxDeg)) {
          ...
          // +1 day: the supplement window is anchored at now() while
          // the cached global rows can be up to 24 h older.  Without
          // the extra day, sample rows in the oldest slice of the
          // cache have no per-path row and "no row" scores as
          // ground-truth-closed, biasing the window start.
          cached.wsprByPath = await fetchWsprByPath(REF_PATHS, days + 1, bboxDeg);
...
// the nearest-in-time station snapshot (foF2, hmF2, foEs) for each
...
export const DEFAULT_CONFIG = {
  ...
  // BAND_SIGMA_DB.
  sigmaScale:              1.0,
  // Multiplier on lLowBandExtraDb (low-band quiet-day D-region).
  // Default 1.0 = production values. Rarely dominates the verdict in
  // current baskets but matters at solar minimum. Exposed here so
  // r7-scan can sweep it during dedicated calibration runs.
  // (A former sibling `esScreenScale` was a phantom: never read by the
  // margin path - lEsScreenDb isn't part of the harness margin - so a
  // sweep over it produced identical results. Removed 2026-08-03.)
  lowBandScale:            1.0,
...
// path. Used by harness-unit.mjs and any caller without precomputed
...
export function replayMarginFromCell(cell, band, config) {
  ...
  // `muf` is returned so consumers can partition samples by f/MUF -
  // scatter-fusion's above-MUF split gated on it while it wasn't
  // returned, which made that subset silently empty in every run.
  return { margin, sigma, muf, lFs, lAbsD, lMuf, lLow, lHop, lAur, noise,
...
function _rotateKc2gIfNewDay() {
  ...
  // mtime = time of the last append.  On the first snapshot of a new
  // UTC day it still carries yesterday's date -> rotate.  (An earlier
  // "mtime younger than 1 h -> skip" guard defeated this at the
  // default 15-min cadence: the file was always <1 h old at midnight,
  // the guard returned early, the append restamped mtime with today,
  // and the archive never rotated.)
...
async function main() {
  ...
  if (WRITE_BASELINE) {
    const baseline = {
      ...
      // The config that actually produced these numbers - a --fusion
      // baseline used to record DEFAULT_CONFIG (fusionEnabled:false)
      // and mislabel itself.
      config,
      f107, f107A,
      n: result.n,
