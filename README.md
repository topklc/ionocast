...
<p align="center">
  <a href="https://github.com/topklc/ionocast/actions/workflows/push.yml"><img src="https://github.com/topklc/ionocast/actions/workflows/push.yml/badge.svg" alt="tests"></a>
...
| CI | GitHub Actions, 4 consolidated workflows | Self-running pipeline; survives months of inattention. |
...
`push.yml` runs server-side, but catches failures before
...
Four consolidated workflows keep the project healthy with no input.

| Workflow | Cron | Purpose |
|---|---|---|
| `push.yml` | every push / PR | Fast unit + i18n + parse-check (~5 min); rebuilds the whitepaper PDF when the LaTeX source changed |
| `daily.yml` | 05/11/17/23 UTC + 06:00 UTC | Tropo GFS grid refresh to R2 (4x daily); WSPR baseline auto-commit, all non-heavy suites, and the lychee link scan (06:00) |
| `monthly.yml` | 09:00 UTC on the 3rd | GIRO station audit, SWPC schema check, tropo Hepburn autotune (ledger + guarded PR), and the full heavy suite (`tune-r7`, `voacap-fixtures`) |
| `annually.yml` | 04:00 UTC 20 October | IMO calendar refresh, citation review, solar-cycle retune review, IGRF / ITU-R / cycle-phase watchdogs |
